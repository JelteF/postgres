...
def test_repl_stats(create_pg):
    node = create_pg(
        "test", conf={"wal_level": "logical", "synchronous_commit": True}
    )

    with node.connect() as conn:
        conn.sql("CREATE TABLE test_repl_stat(col1 int)")
        ...
        conn.sql("INSERT INTO test_repl_stat values(generate_series(1, 5))")
    ...
    assert "Database cluster state:" in out and "shut down" in out, (
        "node shut down ok"
    )
