...
def test_repl_stats(create_pg, pg_bin):
    node = create_pg("test")
    node.append_conf("wal_level = logical", "synchronous_commit = on")
    node.pg_ctl("restart")
    ...
    assert "Database cluster state:" in r.stdout and "shut down" in r.stdout, (
        "node shut down ok"
    )
