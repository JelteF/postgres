...
def auto_explain(pg_server_module):
    ...
    module finishes."""
    with pg_server_module.reloading() as s:
        s.conf.set(
            **{
...
def test_json_format(pg, conn):
    ...
    assert re.search(r'"Query Text": "SELECT \* FROM pg_class;"', log)
    assert not re.search(r'"Query Parameters":', log)
    assert re.search(r'"Node Type": "Seq Scan"[^}]*"Relation Name": "pg_class"', log, re.S)
...
def test_extension_options(pg, conn):
    ...
    assert re.search(r"Parallel Safe:", log), "extension option produces per-node output"
    assert re.search(r"Command Type: select", log), "extension option produces per-plan output"
...
def test_suset_parameter_by_non_superuser(pg, conn):
    ...
    with pg.reloading() as s:
        s.hba.prepend(["local", "all", "regress_user1", "trust"])
