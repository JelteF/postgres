...
def test_wal(create_pg):
    ...
    def check_index_replay(name):
        # Wait for the standby to catch up, then compare query results.
        primary.wait_for_catchup(standby)
        assert query_results(primary) == query_results(standby), (
    ...
    check_index_replay("initial")

    # Run 10 cycles of table modification, checking replay after each step.
    for i in range(1, 11):
        primary.sql("DELETE FROM tst WHERE i = $1", i)
        check_index_replay(f"delete {i}")
        primary.sql("VACUUM tst")
        check_index_replay(f"vacuum {i}")
