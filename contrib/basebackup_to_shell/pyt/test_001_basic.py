...
def test_basic(create_pg, tmp_path):
    ...
    with node.connect() as conn:
        conn.sql("CREATE USER backupuser REPLICATION")
        conn.sql("CREATE ROLE trustworthy")

    # Can't use the module without setting basebackup_to_shell.command.
    pg_basebackup.check_all(
        *BASEBACKUP, "--target", "shell", exit_code=1,
        stderr="shell command for backup is not configured", server=node,
    )
    ...
    pg_basebackup.check_all(
        *BASEBACKUP, "--target", "shell:foo", exit_code=1,
    ...
    pg_basebackup.check_all(
        *BASEBACKUP, "--target", "shell", exit_code=1,
        stderr="permission denied to use basebackup_to_shell", server=node,
    )
    ...
    pg_basebackup.check_all(
        *BASEBACKUP, "--target", "shell", exit_code=1,
        stderr="a target detail is required because the configured command "
        "includes %d",
        server=node,
    )
