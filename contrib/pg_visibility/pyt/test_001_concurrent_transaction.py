...
def test_concurrent_transaction(create_pg):
    ...
    standby = create_pg(
        "pgvis_standby", from_backup=backup, streaming_primary=primary
    )
    ...
    with primary.connect() as conn:
        conn.sql("CREATE EXTENSION pg_visibility")
        conn.sql("CREATE TABLE vacuum_test AS SELECT 42 i")
        conn.sql("VACUUM (disable_page_skipping) vacuum_test")

        # No false negatives on the primary.
        assert conn.sql("SELECT * FROM pg_check_visible('vacuum_test')") == []
