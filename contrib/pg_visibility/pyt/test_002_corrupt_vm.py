...
def test_corrupt_vm(create_pg):
    ...
    conn = node.connect()
    blck_size = int(conn.sql("SHOW block_size"))
    ...
    conn.sql("VACUUM (FREEZE, DISABLE_PAGE_SKIPPING) corruption_test")
    ...
    file = conn.sql("SELECT pg_relation_filepath('corruption_test')")

    # Delete the first block so it is skipped (neither visible nor frozen).
    conn.sql("DELETE FROM corruption_test WHERE (ctid::text::point)[0] = 0")
    ...
    conn.sql(f"DELETE FROM corruption_test WHERE ctid in ({in_clause})")
