...
def _corrupt_first_page(node, relname):
    ...
    with open(relpath, "r+b") as f:
        ...
        f.seek(32)
        f.write(struct.pack(
            "<6L", 0xAAA15550, 0xAAA0D550, 0x00010000,
                    ...
                    for startblock in ("NULL", "0"):
                        for endblock in ("NULL", "0"):
                            ...
                            _detects_no_corruption(
                                conn, f"verify_heapam('{relname}', {opts})"
                            )
...
def test_verify_heapam(node):
    ...
    with node.connect() as conn:
        _detects_heap_corruption(conn, "verify_heapam('test')")
        _detects_heap_corruption(conn, "verify_heapam('test', skip := 'all-visible')")
        _detects_heap_corruption(conn, "verify_heapam('test', skip := 'all-frozen')")
        _detects_heap_corruption(conn, "verify_heapam('test', check_toast := false)")
        _detects_heap_corruption(
            conn, "verify_heapam('test', startblock := 0, endblock := 0)"
        )
    ...
    with node.connect() as conn:
        ...
        _detects_no_corruption(conn, "verify_heapam('test', skip := 'all-frozen')")
