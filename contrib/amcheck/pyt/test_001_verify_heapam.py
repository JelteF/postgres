...
@pytest.fixture
def node(create_pg):
    n = create_pg("test", initdb_opts=["--no-data-checksums"])
    n.append_conf("autovacuum=off")
    n.pg_ctl("restart")
...
def _verify(node, function):
    """Run a verify_heapam() variant and return its report rows as a list."""
    rows = node.sql(f"SELECT * FROM {function}")
    if rows == [] or rows is None:
        return []
    return rows if isinstance(rows, list) else [rows]


def _detects_no_corruption(node, function):
    assert _verify(node, function) == [], function


def _detects_heap_corruption(node, function):
    text = "\n".join(str(r) for r in _verify(node, function))
