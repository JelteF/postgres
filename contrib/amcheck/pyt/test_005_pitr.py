...
def test_pitr(create_pg):
    origin = create_pg(
        "origin", allows_streaming=True, archiving=True, conf=["autovacuum = off"]
    )
    ...
    with origin.connect() as c:
        c.sql("SET synchronous_commit = off")
        c.sql("VACUUM (VERBOSE, INDEX_CLEANUP ON) not_leftmost")
        c.sql("CREATE TABLE XLogFlush ()")
        c.sql("DROP TABLE XLogFlush")
        unlink_lsn = c.sql(
    ...
    # promote. create_pg starts it, so the recovery target is set up front.
    replica = create_pg(
        "replica",
        from_backup=backup,
        restoring=origin,
        conf=[
            f"recovery_target_lsn = '{unlink_lsn}'",
    ...
    with replica.connect() as c:
        c.sql("SET client_min_messages = 'debug1'")
        with warnings.catch_warnings(record=True) as caught:
            warnings.simplefilter("always")
            c.sql("SELECT bt_index_parent_check('not_leftmost_pk', true)")
        ...
        c.sql("SET client_min_messages = 'debug1'")
        c.sql("SELECT bt_index_check('not_leftmost_pk', true)")
