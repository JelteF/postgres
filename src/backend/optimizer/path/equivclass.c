						 ...
						 bool create_it)
{
	 ...
	 */
	Relids		expr_relids = pull_varnos((Node *) expr);
...
find_em_expr_usable_for_sorting_rel(EquivalenceClass *ec, RelOptInfo *rel)
{
	...
	foreach(lc_em, ec->ec_members)
	{
		...
		/*
		 * If, however, it's volatile, we have to verify that the equivalence
		 * member's expr is already generated in the relation's target (we do
		 * strip relabels first from both expressions, which is cheap and
		 * might allow us to match more expressions).
		 */
							 ...
							 bool outer_on_left)
{
	...
	foreach(lc1, root->eq_classes)
	{
		...
		foreach(lc2, cur_ec->ec_members)
		{
			...
			RestrictInfo *newrinfo = build_implied_join_equality(eq_op,
																 cur_ec->ec_collation,
																 innervar,
																 cur_em->em_expr,
