					 ...
					 NestPath *best_path)
{
	...
	Plan	   *outer_plan = create_plan_recurse(root, best_path->outerjoinpath, 0);

	/* For a nestloop, include outer relids in curOuterRels for inner side */
	root->curOuterRels = bms_union(root->curOuterRels,
								   best_path->outerjoinpath->parent->relids);

	Plan	   *inner_plan = create_plan_recurse(root, best_path->innerjoinpath, 0);
	 ...
	 */
	Relids		outerrelids = best_path->outerjoinpath->parent->relids;
	...
	foreach(i, pathkeys)
	{
		...
		if (!tle)
		{
			...
			foreach(j, ec->ec_members)
			{
				...
				sortexpr = em->em_expr;
				List	   *exprvars = pull_var_clause((Node *) sortexpr,
