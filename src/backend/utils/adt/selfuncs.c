								...
								List **varinfos, double *ndistinct)
{
	...
	int			nmatches = 1;				/* we require at least two matches */
	foreach(lc, rel->statlist)
	{
		...
		Bitmapset  *shared = bms_intersect(info->keys, attnums);
		int			nshared = bms_num_members(shared);
				 ...
				 VariableStatData *vardata)
{
	 ...
	 */
	Relids		varnos = pull_varnos(basenode);
