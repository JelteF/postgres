...
pg_get_statisticsobjdef(PG_FUNCTION_ARGS)
{
	...
	char	   *res = pg_get_statisticsobj_worker(statextid, true);
...
add_cast_to(StringInfo buf, Oid typid)
{
	...
	char	   *nspname = get_namespace_name(typform->typnamespace);
...
generate_qualified_type_name(Oid typid)
{
	...
	char	   *nspname = get_namespace_name(typtup->typnamespace);
	if (!nspname)
		elog(ERROR, "cache lookup failed for namespace %u",
			 typtup->typnamespace);
