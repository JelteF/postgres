							 ...
							 List *clauses)
{
			...
			if (is_orclause(clause))
			{
				...
				foreach(lc1, ((BoolExpr *) clause)->args)
				{
					...
					if (argsteps != NIL)
					{
						...
						PartitionPruneStep *step = (PartitionPruneStep *) linitial(argsteps);
	...
	else if (generate_opsteps)
	{
		...
		/* Strategy 2 */
		PartitionPruneStep *step = gen_prune_steps_from_opexps(context, keyclauses, nullkeys);
	...
	if (list_length(result) > 1)
	{
		...
		if (step_ids != NIL)
		{
			...
			PartitionPruneStep *step = gen_prune_step_combine(context, step_ids,
															  PARTPRUNE_COMBINE_INTERSECT);
							 ...
							 PruneStepResult **step_results)
{
	...
	if (list_length(cstep->source_stepids) == 0)
	{
		 ...
		 */
		int			rangemax = context->strategy == PARTITION_STRATEGY_RANGE ?
