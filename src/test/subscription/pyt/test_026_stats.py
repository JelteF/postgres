...
def test_stats(create_pg):
    # The tablesync error loop retries with backoff, so this test runs many
    # sequential polls. Give each node a fresh full timeout per poll (as the
    # Perl test does) rather than a single shared per-test deadline.
    ...
    assert subscriber.sql(
        ...
        f"pg_stat_subscription_stats WHERE subname = '{sub1_name}'"
    ) is True, f"Check reset timestamp for '{sub1_name}' is newer after second reset."
    ...
    assert subscriber.sql(
        f"SELECT stats_reset > '{reset_time1}'::timestamptz FROM "
        f"pg_stat_subscription_stats WHERE subname = '{sub1_name}'"
    ) is True, f"Confirm that reset timestamp for '{sub1_name}' is newer after second reset."
    assert subscriber.sql(
        f"SELECT stats_reset > '{reset_time2}'::timestamptz FROM "
        f"pg_stat_subscription_stats WHERE subname = '{sub2_name}'"
    ) is True, f"Confirm that reset timestamp for '{sub2_name}' is newer after second reset."

    # Dropping a subscription removes its stats entry.
    sub1_oid = subscriber.sql(f"SELECT oid FROM pg_subscription WHERE subname = '{sub1_name}'")
    subscriber.sql(f"DROP SUBSCRIPTION {sub1_name}")
    assert subscriber.sql(
        f"SELECT pg_stat_have_stats('subscription', 0, {sub1_oid})"
    ) is False, f"Subscription stats for subscription '{sub1_name}' should be removed."

    sub2_oid = subscriber.sql(f"SELECT oid FROM pg_subscription WHERE subname = '{sub2_name}'")
    ...
    assert subscriber.sql(
        f"SELECT pg_stat_have_stats('subscription', 0, {sub2_oid})"
    ) is False, f"Subscription stats for subscription '{sub2_name}' should be removed."
