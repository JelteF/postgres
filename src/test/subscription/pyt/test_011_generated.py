...
def test_generated(create_pg):
    ...
    with publisher.connect() as pub:
        pub.sql("INSERT INTO tab1 VALUES (7), (8)")
        pub.sql("UPDATE tab1 SET a = 9 WHERE a = 7")
    publisher.wait_for_catchup("sub1")
    assert subscriber.sql("SELECT * FROM tab1 ORDER BY 1") == [
        (1, 22, 33, None), (2, 44, 66, None), (3, 66, 99, None),
        (4, 88, 132, None), (6, 132, 198, None),
        (8, 176, 264, 18), (9, 198, 297, 19),
    ...
    with subscriber.connect() as sub:
        sub.sql("CREATE TABLE tab_gen_to_nogen (a int, b int)")
        sub.sql(
    ...
    with subscriber.connect() as sub:
        sub.sql("CREATE TABLE tab3 (a int, gen1 int, gen2 int)")
        sub.sql(
            f"CREATE SUBSCRIPTION sub1 CONNECTION '{connstr}' PUBLICATION pub1 "
            "WITH (copy_data = true)"
        )
    ...
    publisher.wait_for_catchup("sub1")
    assert subscriber.sql("SELECT * FROM tab3 ORDER BY gen1") == [
        (None, 2, None), (None, 4, None), (None, 6, None), (None, 8, None)
