...
def test_on_error(create_pg):
    # Many sequential polls/waits across three skip cycles; give each a fresh
    # full timeout rather than a single shared per-test deadline.
    ...
    subscriber = create_pg(
        "subscriber",
        conf=["max_prepared_transactions = 10", "track_commit_timestamp = on"],
    )
    ...
    assert subscriber.sql("SELECT COUNT(*) FROM tbl") == 1, "subscription sub replicated data"

    # Plain commit conflict, then skip.
    publisher.sql("BEGIN; INSERT INTO tbl VALUES (1, NULL); COMMIT")
    ...
    publisher.sql("BEGIN; UPDATE tbl SET i = 2; PREPARE TRANSACTION 'gtx'")
