...
def test_add_drop_pub(create_pg):
    ...
    with publisher.connect() as pub:
        pub.sql("CREATE TABLE tab_1 (a int)")
        pub.sql("INSERT INTO tab_1 SELECT generate_series(1,10)")
    subscriber.sql("CREATE TABLE tab_1 (a int)")

    with publisher.connect() as pub:
        pub.sql("CREATE PUBLICATION tap_pub_1 FOR TABLE tab_1")
        pub.sql("CREATE PUBLICATION tap_pub_2")
    subscriber.sql(
    ...
    assert subscriber.sql("SELECT count(*), min(a), max(a) FROM tab_1") == (10, 1, 10), (
        "check initial data is copied to subscriber"
    )

    with publisher.connect() as pub:
        pub.sql("CREATE TABLE tab_2 (a int)")
        pub.sql("INSERT INTO tab_2 SELECT generate_series(1,10)")
    ...
    subscriber.wait_for_subscription_sync(publisher, "tap_sub")
    assert subscriber.sql("SELECT count(*), min(a), max(a) FROM tab_2") == (10, 1, 10), (
        "check initial data is copied to subscriber"
    )
    ...
    subscriber.wait_for_subscription_sync(publisher, "tap_sub")
    assert subscriber.sql("SELECT count(*), min(a), max(a) FROM tab_1") == (20, 1, 10), (
        "check initial data is copied to subscriber"
    )
