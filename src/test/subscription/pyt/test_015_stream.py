...
def test_stream(create_pg):
    publisher = create_pg(
        "publisher", allows_streaming="logical", conf={"logical_decoding_work_mem": "64kB"}
    )
    ...
    def test_streaming(appname, is_parallel):
        ...
        assert subscriber.sql("SELECT count(*), count(c), count(d = 999) FROM test_tab") == (
            3334, 3334, 3334,
    ...
    with subscriber.connect() as sub:
        ...
        sub.sql("CREATE UNIQUE INDEX idx_tab on test_tab_2(a)")
    ...
    h = publisher.connect()
    offset = subscriber.current_log_position()
    h.sql_batch("BEGIN", "INSERT INTO test_tab_2 SELECT i FROM generate_series(1, 5000) s(i)")
