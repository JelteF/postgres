...
def test_stream(create_pg):
    # Long test with background sessions, many waits and reloads; give each node
    # a fresh full timeout per poll rather than a single shared per-test deadline.
    ...
    h = publisher.background()
    offset = subscriber.current_log_position()
    h.sql("BEGIN; INSERT INTO test_tab_2 SELECT i FROM generate_series(1, 5000) s(i)")
