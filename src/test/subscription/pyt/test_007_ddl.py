...
def test_ddl(create_pg):
    ...
    subscriber.sql(f"CREATE SUBSCRIPTION mysub CONNECTION '{connstr}' PUBLICATION mypub")
    ...
    with publisher.connect() as pub:
        pub.sql("DROP PUBLICATION mypub")
        pub.sql("SELECT pg_drop_replication_slot('mysub')")
    ...
    subscriber.sql(f"CREATE SUBSCRIPTION tap_sub CONNECTION '{connstr}' PUBLICATION pub_for_tab")
    ...
    with subscriber.connect() as sub:
        ...
        sub.sql("DROP TABLE test1, test2")
