...
def test_twophase_cascade(create_pg):
    ...
    with node_A.connect() as a:
        a.sql("CREATE TABLE tab_full (a int PRIMARY KEY)")
        a.sql("INSERT INTO tab_full SELECT generate_series(1,10)")
    ...
    with node_A.connect() as a:
        a.sql("CREATE TABLE test_tab (a int primary key, b bytea)")
        a.sql("INSERT INTO test_tab VALUES (1, 'foo'), (2, 'bar')")
    node_B.sql(
    ...
    def both_subscribers_prepared(expected, msg):
        assert node_B.sql("SELECT count(*) FROM pg_prepared_xacts") == expected, f"{msg} B"
        assert node_C.sql("SELECT count(*) FROM pg_prepared_xacts") == expected, f"{msg} C"
    ...
    cascade_catchup()
    assert node_B.sql("SELECT count(*), count(c), count(d = 999) FROM test_tab") == (3334, 3334, 3334), (
        "Rows inserted by 2PC have committed on subscriber B, and extra columns have local defaults"
    )
    assert node_C.sql("SELECT count(*), count(c), count(d = 999) FROM test_tab") == (3334, 3334, 3334), (
    ...
    assert node_B.sql("SELECT count(*) FROM test_tab") == 3, "Rows committed are present on subscriber B"
    assert node_C.sql("SELECT count(*) FROM test_tab where b = 'foobar'") == 1, (
        "Rows committed are present on subscriber C"
    )
    assert node_C.sql("SELECT count(*) FROM test_tab") == 3, "Rows committed are present on subscriber C"

    # ===== cleanup ==========================================================
    with node_C.connect() as c:
        c.sql("DROP SUBSCRIPTION tap_sub_C")
        assert c.sql("SELECT count(*) FROM pg_subscription") == 0, (
            "check subscription was dropped on subscriber node C"
        )
        assert c.sql("SELECT count(*) FROM pg_subscription_rel") == 0, (
            "check subscription relation status was dropped on subscriber node C"
        )
        assert c.sql("SELECT count(*) FROM pg_replication_origin") == 0, (
    ...
    with node_B.connect() as b:
        b.sql("DROP SUBSCRIPTION tap_sub_B")
        assert b.sql("SELECT count(*) FROM pg_subscription") == 0, (
            "check subscription was dropped on subscriber node B"
        )
        assert b.sql("SELECT count(*) FROM pg_subscription_rel") == 0, (
            "check subscription relation status was dropped on subscriber node B"
        )
        assert b.sql("SELECT count(*) FROM pg_replication_origin") == 0, (
