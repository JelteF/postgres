...
def test_twophase_cascade(create_pg):
    # Long cascade test with many sequential waits; give each node a fresh full
    # timeout per poll rather than a single shared per-test deadline.
    ...
    node_A.sql("BEGIN; INSERT INTO tab_full VALUES (11); PREPARE TRANSACTION 'test_prepared_tab_full'")
    cascade_catchup()
    ...
    node_A.sql("BEGIN; INSERT INTO tab_full VALUES (12); PREPARE TRANSACTION 'test_prepared_tab_full'")
    cascade_catchup()
    both_subscribers_prepared(1, "transaction is prepared on subscriber")
