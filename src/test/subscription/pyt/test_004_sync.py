...
def test_sync(create_pg):
    ...
    with publisher.connect() as pub:
        ...
        pub.sql("INSERT INTO tab_rep SELECT generate_series(1,10)")
    ...
    assert subscriber.sql("SELECT count(*) FROM tab_rep") == 10, "initial data synced for first sub"
    ...
    assert subscriber.sql("SELECT count(*) FROM tab_rep") == 20, "initial data synced for second sub"
    ...
    with subscriber.connect() as sub:
        ...
        sub.sql("DROP SUBSCRIPTION tap_sub2")
        assert sub.sql("SELECT count(*) FROM pg_subscription") == 0, (
            "second and third sub are dropped"
        )

        sub.sql("DELETE FROM tab_rep")
        sub.sql(f"CREATE SUBSCRIPTION tap_sub CONNECTION '{connstr}' PUBLICATION tap_pub")
    subscriber.wait_for_subscription_sync()
    assert subscriber.sql("SELECT count(*) FROM tab_rep") == 20, "initial data synced for fourth sub"
