...
def test_collation(create_pg):
    ...
    with publisher.connect() as pub:
        pub.sql("CREATE TABLE tab1 (a text PRIMARY KEY, b text)")
        pub.sql(r"INSERT INTO tab1 VALUES (U&'\00E4bc', 'foo')")
    with subscriber.connect() as sub:
        sub.sql("CREATE TABLE tab1 (a text COLLATE ctest_nondet PRIMARY KEY, b text)")
        sub.sql(r"INSERT INTO tab1 VALUES (U&'\0061\0308bc', 'foo')")

    # Table with replica identity full.
    with publisher.connect() as pub:
        pub.sql("CREATE TABLE tab2 (a text, b text)")
        pub.sql("ALTER TABLE tab2 REPLICA IDENTITY FULL")
        pub.sql(r"INSERT INTO tab2 VALUES (U&'\00E4bc', 'foo')")
    with subscriber.connect() as sub:
        sub.sql("CREATE TABLE tab2 (a text COLLATE ctest_nondet, b text)")
        sub.sql("ALTER TABLE tab2 REPLICA IDENTITY FULL")
        sub.sql(r"INSERT INTO tab2 VALUES (U&'\0061\0308bc', 'foo')")
