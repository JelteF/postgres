...
def test_rep_changes_for_schema(create_pg):
    ...
    with publisher.connect() as pub:
        pub.sql("CREATE SCHEMA sch1")
        pub.sql("CREATE TABLE sch1.tab1 AS SELECT generate_series(1,10) AS a")
        pub.sql("CREATE TABLE sch1.tab2 AS SELECT generate_series(1,10) AS a")
        pub.sql(
        ...
        pub.sql("INSERT INTO sch1.tab1_parent values (1),(4)")

    with subscriber.connect() as sub:
        sub.sql("CREATE SCHEMA sch1")
        sub.sql("CREATE TABLE sch1.tab1 (a int)")
        sub.sql("CREATE TABLE sch1.tab2 (a int)")
        sub.sql(
            "CREATE TABLE sch1.tab1_parent (a int PRIMARY KEY, b text) PARTITION BY LIST (a)"
        )
        sub.sql(
    ...
    assert subscriber.sql("SELECT count(*), min(a), max(a) FROM sch1.tab1") == (10, 1, 10), (
        "check rows on subscriber catchup"
    )
    assert subscriber.sql("SELECT count(*), min(a), max(a) FROM sch1.tab2") == (10, 1, 10), (
        "check rows on subscriber catchup"
    )
    ...
    with publisher.connect() as pub:
        pub.sql("INSERT INTO sch1.tab1 VALUES(generate_series(11,20))")
        pub.sql("INSERT INTO sch1.tab1_parent values (2),(5)")
    publisher.wait_for_catchup("tap_sub_schema")
    assert subscriber.sql("SELECT count(*), min(a), max(a) FROM sch1.tab1") == (20, 1, 20), (
    ...
    publisher.wait_for_catchup("tap_sub_schema")
    assert subscriber.sql("SELECT count(*), min(a), max(a) FROM sch1.tab3") == (11, 1, 11), (
        "check rows on subscriber catchup"
    )

    # Moving a table out of the schema stops its replication.
    with publisher.connect() as pub:
        pub.sql("ALTER TABLE sch1.tab3 SET SCHEMA public")
        pub.sql("INSERT INTO public.tab3 VALUES(12)")
    publisher.wait_for_catchup("tap_sub_schema")
    assert subscriber.sql("SELECT count(*), min(a), max(a) FROM sch1.tab3") == (11, 1, 11), (
        "check replicated inserts on subscriber"
    )
    ...
    publisher.wait_for_catchup("tap_sub_schema")
    assert subscriber.sql("SELECT count(*), min(a), max(a) FROM sch1.tab1") == (21, 1, 21), (
        "check replicated inserts on subscriber"
    )
