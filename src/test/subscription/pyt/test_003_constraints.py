...
def test_constraints(create_pg):
    ...
    with publisher.connect() as pub:
        pub.sql("CREATE TABLE tab_fk (bid int PRIMARY KEY)")
        pub.sql(
    ...
    with subscriber.connect() as sub:
        sub.sql("CREATE TABLE tab_fk (bid int PRIMARY KEY)")
        sub.sql(
    ...
    with publisher.connect() as pub:
        pub.sql("INSERT INTO tab_fk (bid) VALUES (1)")
    ...
    assert subscriber.sql("SELECT count(*), min(bid), max(bid) FROM tab_fk") == (1, 1, 1), (
        "check replicated tab_fk inserts on subscriber"
    )
    assert subscriber.sql("SELECT count(*), min(bid), max(bid) FROM tab_fk_ref") == (1, 1, 1), (
        "check replicated tab_fk_ref inserts on subscriber"
    )

    # FK is not enforced on the subscriber.
    with publisher.connect() as pub:
        pub.sql("DROP TABLE tab_fk CASCADE")
        pub.sql("INSERT INTO tab_fk_ref (id, bid) VALUES (2, 2)")
    publisher.wait_for_catchup("tap_sub")
    assert subscriber.sql("SELECT count(*), min(bid), max(bid) FROM tab_fk_ref") == (2, 1, 2), (
    ...
    publisher.wait_for_catchup("tap_sub")
    assert subscriber.sql("SELECT count(*), min(id), max(id) FROM tab_fk_ref") == (2, 1, 2), (
