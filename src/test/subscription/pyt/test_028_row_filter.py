...
def test_row_filter(create_pg):
    ...
    with publisher.connect() as pub:
        pub.sql("INSERT INTO tab_rf_x (x) VALUES (0), (5), (10), (15), (20)")
        pub.sql("CREATE PUBLICATION tap_pub_x FOR TABLE tab_rf_x WHERE (x > 10)")
        pub.sql("CREATE PUBLICATION tap_pub_forall FOR ALL TABLES")
    subscriber.sql(
    ...
    with publisher.connect() as pub:
        pub.sql("DROP PUBLICATION tap_pub_forall")
        pub.sql("DROP PUBLICATION tap_pub_x")
        pub.sql("DROP TABLE tab_rf_x")
    with subscriber.connect() as sub:
        sub.sql("DROP SUBSCRIPTION tap_sub")
        sub.sql("DROP TABLE tab_rf_x")
    ...
    with publisher.connect() as pub:
        pub.sql("INSERT INTO schema_rf_x.tab_rf_x (x) VALUES (0), (5), (10), (15), (20)")
        pub.sql("INSERT INTO schema_rf_x.tab_rf_partitioned (x) VALUES (1), (20)")
        pub.sql(
    ...
    with publisher.connect() as pub:
        pub.sql("INSERT INTO schema_rf_x.tab_rf_x (x) VALUES (-99), (99)")
        pub.sql("INSERT INTO schema_rf_x.tab_rf_partitioned (x) VALUES (5), (25)")
    publisher.wait_for_catchup("tap_sub")
    assert subscriber.sql("SELECT count(x) FROM schema_rf_x.tab_rf_x") == 7, (
        "check table tab_rf_x should not be filtered"
    )
    assert subscriber.sql("SELECT * FROM public.tab_rf_partition ORDER BY 1") == [20, 25], (
        "check table tab_rf_partition should be filtered"
    )

    with publisher.connect() as pub:
        pub.sql("DROP PUBLICATION tap_pub_allinschema")
        pub.sql("DROP PUBLICATION tap_pub_x")
        pub.sql("DROP TABLE public.tab_rf_partition")
        pub.sql("DROP TABLE schema_rf_x.tab_rf_partitioned")
        pub.sql("DROP TABLE schema_rf_x.tab_rf_x")
        pub.sql("DROP SCHEMA schema_rf_x")
    with subscriber.connect() as sub:
        sub.sql("DROP SUBSCRIPTION tap_sub")
        sub.sql("DROP TABLE public.tab_rf_partition")
        sub.sql("DROP TABLE schema_rf_x.tab_rf_partitioned")
        sub.sql("DROP TABLE schema_rf_x.tab_rf_x")
        sub.sql("DROP SCHEMA schema_rf_x")
    ...
    with publisher.connect() as pub:
        ...
        pub.sql("CREATE PUBLICATION tap_pub_2 FOR TABLE tab_rowfilter_2 WHERE (c % 3 = 0)")
        pub.sql("CREATE PUBLICATION tap_pub_3 FOR TABLE tab_rowfilter_partitioned")
        pub.sql(
            "ALTER PUBLICATION tap_pub_3 ADD TABLE tab_rowfilter_less_10k WHERE (a < 6000)"
        )
        pub.sql("CREATE PUBLICATION tap_pub_not_used FOR TABLE tab_rowfilter_1 WHERE (a < 0)")
        pub.sql("CREATE PUBLICATION tap_pub_4a FOR TABLE tab_rowfilter_4 WHERE (c % 2 = 0)")
        pub.sql("CREATE PUBLICATION tap_pub_4b FOR TABLE tab_rowfilter_4")
        pub.sql("CREATE PUBLICATION tap_pub_5a FOR TABLE tab_rowfilter_partitioned_2")
        pub.sql("CREATE PUBLICATION tap_pub_5b FOR TABLE tab_rowfilter_partition WHERE (a > 10)")
        pub.sql(
        ...
        pub.sql("CREATE PUBLICATION tap_pub_virtual FOR TABLE tab_rowfilter_virtual WHERE (y > 10)")
    ...
    ) == (13, 2, 20), "check initial data copy from table tab_rowfilter_2"

    # tab_rowfilter_4: same table in two pubs, only one filtered → no filter.
    assert subscriber.sql(
        "SELECT count(c), min(c), max(c) FROM tab_rowfilter_4"
    ) == (10, 1, 10), "check initial data copy from table tab_rowfilter_4"
    ...
    assert subscriber.sql("SELECT a, b FROM tab_rowfilter_greater_10k ORDER BY 1, 2") == [
        (15000, 102),
        (16000, 103),
    ...
    assert subscriber.sql("SELECT a FROM tab_rowfilter_inherited ORDER BY a") == [20, 30, 40], (
    ...
    assert subscriber.sql("SELECT id, x FROM tab_rowfilter_virtual ORDER BY id") == (3, 6), (
    ...
    assert subscriber.sql(
        "SELECT count(c), min(c), max(c) FROM tab_rowfilter_2"
    ) == (16, 2, 24), "check replicated rows to tab_rowfilter_2"

    # tab_rowfilter_4: all initial rows plus (0, 11, 12).
    assert subscriber.sql(
        "SELECT count(c), min(c), max(c) FROM tab_rowfilter_4"
    ) == (13, 0, 12), "check replicated rows to tab_rowfilter_4"
    ...
    with publisher.connect() as pub:
        pub.sql("ALTER PUBLICATION tap_pub_3 SET (publish_via_partition_root = true)")
        pub.sql(
            "ALTER PUBLICATION tap_pub_3 SET TABLE tab_rowfilter_partitioned WHERE (a < 5000), "
            "tab_rowfilter_less_10k WHERE (a < 6000)"
        )
    with subscriber.connect() as sub:
        sub.sql("TRUNCATE TABLE tab_rowfilter_partitioned")
        sub.sql("ALTER SUBSCRIPTION tap_sub REFRESH PUBLICATION WITH (copy_data = true)")
    ...
    assert subscriber.sql("SELECT a, b FROM tab_rowfilter_partitioned ORDER BY 1, 2") == [
        (1, 100),
        (2, 200),
        (4000, 400),
        (4001, 30),
