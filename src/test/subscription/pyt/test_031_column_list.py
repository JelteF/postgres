...
def test_column_list(create_pg):
    ...
    subscriber = create_pg(
        "subscriber", conf={"max_logical_replication_workers": 6}
    )
    ...
    publisher.sql_batch("INSERT INTO tab1 VALUES (1, 2, 3)", "INSERT INTO tab1 VALUES (4, 5, 6)")
    publisher.sql_batch("INSERT INTO tab3 VALUES (1, 2, 3)", "INSERT INTO tab3 VALUES (4, 5, 6)")
    publisher.sql_batch(
    ...
    publisher.sql_batch("INSERT INTO tab1 VALUES (2, 3, 4)", "INSERT INTO tab1 VALUES (5, 6, 7)")
    publisher.sql_batch("INSERT INTO tab3 VALUES (2, 3, 4)", "INSERT INTO tab3 VALUES (5, 6, 7)")
    publisher.sql_batch(
    ...
    with publisher.connect() as pub:
        pub.sql('UPDATE tab1 SET "B" = 2 * "B" where a = 1')
        pub.sql("UPDATE tab1 SET c = 2*c where a = 4")
        pub.sql("UPDATE tab3 SET \"B\" = \"B\" || ' updated' where \"a'\" = 4")
        pub.sql("UPDATE tab3 SET \"c'\" = 2 * \"c'\" where \"a'\" = 1")
        pub.sql("UPDATE tab4 SET b = 'blue', c = c * 2, d = d || ' updated' where a = 1")
        pub.sql("UPDATE tab4 SET b = 'red', c = c * 2, d = d || ' updated' where a = 2")
    publisher.wait_for_catchup("sub1")
    ...
    publisher.sql_batch("UPDATE tab2 SET c = 5 where a = 1", "UPDATE tab2 SET b = 'xyz' where a = 2")
    publisher.wait_for_catchup("sub1")
    assert subscriber.sql("SELECT * FROM tab2 ORDER BY a") == [
        (1, "abc"),
        (2, "xyz"),
    ...
    with subscriber.connect() as sub:
        sub.sql("CREATE TABLE tab5 (a int PRIMARY KEY, b int, d int)")
        sub.sql("DROP SUBSCRIPTION sub1")
        sub.sql(f"CREATE SUBSCRIPTION sub1 CONNECTION '{connstr}' PUBLICATION pub2, pub3")
    subscriber.wait_for_subscription_sync(publisher, "sub1")

    publisher.sql_batch("INSERT INTO tab5 VALUES (3, 33, 333, 3333)", "INSERT INTO tab5 VALUES (4, 44, 444, 4444)")
    publisher.wait_for_catchup("sub1")
    ...
    with subscriber.connect() as sub:
        sub.sql("CREATE TABLE tab6 (a int PRIMARY KEY, b int, c int, d int)")
        sub.sql("DROP SUBSCRIPTION sub1")
        sub.sql(f"CREATE SUBSCRIPTION sub1 CONNECTION '{connstr}' PUBLICATION pub4")
    subscriber.wait_for_subscription_sync()

    publisher.sql_batch("INSERT INTO tab6 VALUES (2, 33, 444, 5555)", "UPDATE tab6 SET b = b * 2, c = c * 3, d = d * 4")
    publisher.wait_for_catchup("sub1")
    ...
    publisher.sql_batch("ALTER TABLE tab6 DROP CONSTRAINT tab6_pkey", "ALTER TABLE tab6 ADD PRIMARY KEY (b)")
    subscriber.sql_batch("ALTER TABLE tab6 DROP CONSTRAINT tab6_pkey", "ALTER TABLE tab6 ADD PRIMARY KEY (b)")
    subscriber.sql("ALTER SUBSCRIPTION sub1 REFRESH PUBLICATION")
    subscriber.wait_for_subscription_sync()

    publisher.sql_batch("INSERT INTO tab6 VALUES (3, 55, 666, 8888)", "UPDATE tab6 SET b = b * 2, c = c * 3, d = d * 4")
    publisher.wait_for_catchup("sub1")
    assert subscriber.sql("SELECT * FROM tab6 ORDER BY a") == [
        (1, 88, None, None),
        (2, 132, None, None),
    ...
    with subscriber.connect() as sub:
        sub.sql("CREATE TABLE tab7 (a int PRIMARY KEY, b int, c int, d int)")
        sub.sql("DROP SUBSCRIPTION sub1")
        sub.sql(f"CREATE SUBSCRIPTION sub1 CONNECTION '{connstr}' PUBLICATION pub5")
    subscriber.wait_for_subscription_sync()

    publisher.sql_batch("INSERT INTO tab7 VALUES (2, 33, 444, 5555)", "UPDATE tab7 SET b = b * 2, c = c * 3, d = d * 4")
    publisher.wait_for_catchup("sub1")
    ...
    publisher.sql_batch("ALTER TABLE tab7 DROP CONSTRAINT tab7_pkey", "ALTER TABLE tab7 ADD PRIMARY KEY (a, b)")
    publisher.sql_batch("INSERT INTO tab7 VALUES (3, 55, 666, 7777)", "UPDATE tab7 SET b = b * 2, c = c * 3, d = d * 4")
    publisher.wait_for_catchup("sub1")
    assert subscriber.sql("SELECT * FROM tab7 ORDER BY a") == [
        (1, 88, None, None),
        (2, 132, None, None),
    ...
    with subscriber.connect() as sub:
        sub.sql("DROP SUBSCRIPTION sub1")
        sub.sql(f"CREATE SUBSCRIPTION sub1 CONNECTION '{connstr}' PUBLICATION pub6")
    subscriber.wait_for_subscription_sync()

    publisher.sql_batch("INSERT INTO test_part_a VALUES (2, 5)", "INSERT INTO test_part_a VALUES (7, 6)")
    publisher.wait_for_catchup("sub1")
    ...
    with subscriber.connect() as sub:
        sub.sql("DROP SUBSCRIPTION sub1")
        sub.sql(f"CREATE SUBSCRIPTION sub1 CONNECTION '{connstr}' PUBLICATION pub7")
    subscriber.wait_for_subscription_sync()

    publisher.sql_batch("INSERT INTO test_part_b VALUES (2, 3)", "INSERT INTO test_part_b VALUES (7, 4)")
    publisher.wait_for_catchup("sub1")
    ...
    with subscriber.connect() as sub:
        sub.sql("DROP SUBSCRIPTION sub1")
        sub.sql(f"CREATE SUBSCRIPTION sub1 CONNECTION '{connstr}' PUBLICATION pub8")
    subscriber.wait_for_subscription_sync()

    publisher.sql_batch("INSERT INTO test_part_c VALUES (3, 7, 8)", "INSERT INTO test_part_c VALUES (4, 9, 10)")
    publisher.wait_for_catchup("sub1")
    ...
    with subscriber.connect() as sub:
        sub.sql("ALTER SUBSCRIPTION sub1 REFRESH PUBLICATION")
        sub.sql("TRUNCATE test_part_c")
    subscriber.wait_for_subscription_sync()

    publisher.sql("TRUNCATE test_part_c")
    publisher.sql_batch("INSERT INTO test_part_c VALUES (1, 3, 5)", "INSERT INTO test_part_c VALUES (2, 4, 6)")
    publisher.wait_for_catchup("sub1")
    assert subscriber.sql("SELECT * FROM test_part_c ORDER BY a, b") == [
        (1, None, None),
        (2, 4, None),
    ...
    with subscriber.connect() as sub:
        sub.sql("DROP SUBSCRIPTION sub1")
        sub.sql(f"CREATE SUBSCRIPTION sub1 CONNECTION '{connstr}' PUBLICATION pub9")
    subscriber.wait_for_subscription_sync()
    ...
    with subscriber.connect() as sub:
        sub.sql("CREATE TABLE test_mix_2 (a int PRIMARY KEY, b int, c int)")
        sub.sql("DROP SUBSCRIPTION sub1")
        sub.sql(
    ...
    with subscriber.connect() as sub:
        sub.sql("DROP SUBSCRIPTION sub1")
        sub.sql("CREATE TABLE test_mix_3 (a int PRIMARY KEY, b int, c int)")
    publisher.sql_batch(
    ...
    publisher.sql_batch("INSERT INTO test_root VALUES (2, 3, 4)", "INSERT INTO test_root VALUES (11, 21, 31)")
    publisher.wait_for_catchup("sub1")
    ...
    subscriber.sql("DROP SUBSCRIPTION sub1")
    subscriber.sql(f"CREATE SUBSCRIPTION sub1 CONNECTION '{connstr}' PUBLICATION pub1, pub2")
    subscriber.wait_for_subscription_sync()
    ...
    with subscriber.connect() as sub:
        sub.sql("DROP SUBSCRIPTION sub1")
        sub.sql(f"CREATE SUBSCRIPTION sub1 CONNECTION '{connstr}' PUBLICATION pub3")
    subscriber.wait_for_subscription_sync()
    ...
    publisher.sql("SELECT * FROM pg_create_logical_replication_slot('test_slot', 'pgoutput')")
    publisher.sql_batch("UPDATE test_oldtuple_col SET a = 2", "DELETE FROM test_oldtuple_col")
    ...
    with subscriber.connect() as sub:
        sub.sql("DROP SUBSCRIPTION sub1")
        sub.sql("CREATE TABLE test_mix_4 (a int PRIMARY KEY, b int, c int, d int)")
        sub.sql(
            f"CREATE SUBSCRIPTION sub1 CONNECTION '{connstr}' PUBLICATION pub_mix_7, pub_mix_8"
        )
    subscriber.wait_for_subscription_sync()
    assert subscriber.sql("SELECT * FROM test_mix_4 ORDER BY a") == (1, 2, None, None), (
    ...
    with subscriber.connect() as sub:
        sub.sql("DROP SUBSCRIPTION sub1")
        sub.sql("CREATE TABLE test_mix_1 (a int PRIMARY KEY, b int, c int)")
    with pytest.raises(
        LibpqError,
