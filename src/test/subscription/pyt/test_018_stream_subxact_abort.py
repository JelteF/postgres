...
def test_stream_subxact_abort(create_pg):
    ...
    with subscriber.connect() as sub:
        sub.sql("CREATE TABLE test_tab (a int primary key, b text, c INT, d INT, e INT)")
    ...
    def test_streaming(is_parallel):
        ...
        if is_parallel:
            subscriber.wait_for_log(
                r"DEBUG: ( [A-Z0-9]+:)? finished processing the STREAM COMMIT command", offset
            )
        ...
        assert count_tab() == (7, 0), "check rollback to savepoint was reflected on subscriber"
        ...
        if is_parallel:
            subscriber.wait_for_log(
                r"DEBUG: ( [A-Z0-9]+:)? finished processing the STREAM ABORT command", offset
            )
