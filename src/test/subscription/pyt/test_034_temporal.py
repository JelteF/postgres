...
def test_temporal(create_pg):
    ...
    publisher.sql("ALTER TABLE temporal_pk REPLICA IDENTITY USING INDEX temporal_pk_pkey")
    publisher.sql(
    ...
    subscriber.sql("ALTER TABLE temporal_pk REPLICA IDENTITY USING INDEX temporal_pk_pkey")
    subscriber.sql(
        "CREATE TABLE temporal_unique (id int4range NOT NULL, valid_at daterange NOT NULL, "
        "a text, UNIQUE (id, valid_at WITHOUT OVERLAPS))"
    )
