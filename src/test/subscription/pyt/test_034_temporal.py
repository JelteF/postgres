...
def test_temporal(create_pg):
        ...
        for node in (publisher, subscriber):
            with node.connect() as conn:
                for t, cols in defs.items():
                    conn.sql(f"CREATE TABLE {t} ({cols})")
                    if identity:
                        conn.sql(f"ALTER TABLE {t} REPLICA IDENTITY {identity}")
    ...
    def setup_pub_sub():
        with publisher.connect() as pub:
            ...
            pub.sql("CREATE PUBLICATION pub1 FOR ALL TABLES")
    ...
    def drop_everything():
        with publisher.connect() as pub, subscriber.connect() as sub:
            for t in ("temporal_no_key", "temporal_pk", "temporal_unique"):
                pub.sql(f"DROP TABLE IF EXISTS {t}")
                sub.sql(f"DROP TABLE IF EXISTS {t}")
            pub.sql("DROP PUBLICATION pub1")
    ...
    assert select("temporal_no_key") == INSERTS_ONLY, "replicated temporal_no_key DEFAULT"
    ...
    publisher.wait_for_catchup("sub1")
    assert select("temporal_unique") == INSERTS_ONLY, "replicated temporal_unique DEFAULT"
    drop_everything()
    ...
    with publisher.connect() as pub:
        ...
        pub.sql("ALTER TABLE temporal_pk REPLICA IDENTITY USING INDEX temporal_pk_pkey")
        pub.sql(
    ...
    with subscriber.connect() as sub:
        ...
        sub.sql("ALTER TABLE temporal_pk REPLICA IDENTITY USING INDEX temporal_pk_pkey")
        sub.sql(
            "CREATE TABLE temporal_unique (id int4range NOT NULL, valid_at daterange NOT NULL, "
            "a text, UNIQUE (id, valid_at WITHOUT OVERLAPS))"
        )
