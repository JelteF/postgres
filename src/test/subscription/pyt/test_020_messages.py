...
def test_messages(create_pg):
    publisher = create_pg("publisher", allows_streaming="logical", conf={"autovacuum": False})
    ...
    with publisher.connect() as pub:
        ...
        pub.sql("INSERT INTO tab_test VALUES (1)")
        message_lsn = pub.sql(
            "SELECT pg_logical_emit_message(false, 'pgoutput', 'a non-transactional message')"
        )
        pub.sql("INSERT INTO tab_test VALUES (2)")
        assert pub.sql(
