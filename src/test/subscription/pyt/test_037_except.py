...
def test_except(create_pg):
    ...
    ) is True, "no changes for EXCEPT-clause tables present in the slot"

    # ONLY parent1 was excluded, so child1 is still published.
    publisher.sql("INSERT INTO child1 VALUES(generate_series(11,20), generate_series(11,20))")
    publisher.wait_for_catchup("tap_sub")
    with subscriber.connect() as sub:
        assert sub.sql("SELECT count(*) FROM tab1") == 0, "check replicated inserts"
        assert sub.sql("SELECT count(*) FROM child") == 0, "check replicated inserts"
        assert sub.sql("SELECT count(*) FROM child1") == 10, "check replicated inserts"
    ...
    subscriber.wait_for_subscription_sync(publisher, "tap_sub")
    with subscriber.connect() as sub:
        assert sub.sql("SELECT count(*) FROM tab2") == 0, (
            "no initial data copied for EXCEPT-clause tables"
        )
        assert sub.sql("SELECT count(*) FROM tab1") == 20, (
            "data copied as tab1 was removed from EXCEPT clause"
        )

    subscriber.sql("DROP SUBSCRIPTION tap_sub")
    subscriber.sql_batch("TRUNCATE TABLE tab1", "DROP TABLE parent, parent1, child, child1, tab2")
    publisher.sql("DROP PUBLICATION tap_pub")
    publisher.sql_batch("TRUNCATE TABLE tab1", "DROP TABLE parent, parent1, child, child1, tab2")
    ...
    def test_except_root_partition(pubviaroot):
        ...
        with publisher.connect() as pub:
            ...
            pub.sql("INSERT INTO root1 VALUES (2), (102)")
            pub.sql(
        ...
        with subscriber.connect() as sub:
            for table in ("root1", "part1", "part2", "part2_1"):
                assert sub.sql(f"SELECT count(*) FROM {table}") == 0, (
    ...
    publisher.sql_batch("CREATE PUBLICATION tap_pub2 FOR ALL TABLES", "INSERT INTO tab1 VALUES(1)")
