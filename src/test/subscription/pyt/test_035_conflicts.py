...
def test_conflicts(create_pg):
    ...
    with publisher.connect() as pub:
        pub.sql("CREATE TABLE conf_tab (a int PRIMARY KEY, b int UNIQUE, c int UNIQUE)")
        pub.sql("CREATE TABLE conf_tab_2 (a int PRIMARY KEY, b int UNIQUE, c int UNIQUE)")
    ...
    assert subscriber.sql_oneshot(
        ...
        dbname="clt_ts_test",
    ) == 0, "ALTER TABLE ALL IN TABLESPACE skips the conflict log table"
