...
def test_conflicts(create_pg):
    # Long test with bidirectional replication, restarts and many waits; give
    # each node a fresh full timeout per poll rather than one shared deadline.
    ...
    publisher.sql("CREATE TABLE conf_tab (a int PRIMARY KEY, b int UNIQUE, c int UNIQUE)")
    publisher.sql("CREATE TABLE conf_tab_2 (a int PRIMARY KEY, b int UNIQUE, c int UNIQUE)")
    subscriber.sql("CREATE TABLE conf_tab (a int PRIMARY key, b int UNIQUE, c int UNIQUE)")
    subscriber.sql(
        """
    ...
    node_A.append_conf(
        "track_commit_timestamp = on", "autovacuum = off", "log_min_messages = 'debug2'"
    )
    ...
    with pytest.raises(
        LibpqError, match=r'cannot set option "retain_dead_tuples" for enabled subscription'
    ):
    ...
    assert node_B.sql("SELECT * FROM tab") == [(1, 1), (2, 2)], "check replicated insert on node B"
    ...
    node_A.sql(f"ALTER SUBSCRIPTION {subname_AB} REFRESH PUBLICATION WITH (copy_data = false)")
    ...
    if injection_points_supported:
        ...
        node_B.sql("TRUNCATE tab; INSERT INTO tab VALUES(1, 1)")
        node_B.wait_for_catchup(subname_AB)
        ...
        pub_session = node_B.background()
        pub_session.sql(
            "BEGIN; UPDATE tab SET b = 2 WHERE a = 1; "
            "PREPARE TRANSACTION 'txn_with_later_commit_ts'"
        )
        commit_future = pub_session.asql("COMMIT PREPARED 'txn_with_later_commit_ts'")
        node_B.wait_for_event("client backend", "commit-after-delay-checkpoint")
        assert node_B.sql("SELECT * FROM tab WHERE a = 1") == (1, 1), "publisher sees the old row"
        ...
        assert node_B.sql("SELECT * FROM tab WHERE a = 1") == (1, 2), "publisher sees the new row"
        node_B.wait_for_catchup(subname_AB)
        assert re.search(
            r'conflict detected on relation "public.tab": conflict=update_deleted.*\n'
        ...
        ) is True, "pub UPDATE's timestamp is later than that of sub's DELETE"
    ...
    ) is False, "retention is inactive"
