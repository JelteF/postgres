...
def test_twophase(create_pg):
    ...
    # ===== empty prepared transaction =======================================
    # A transaction that is assigned an XID but makes no change decoded by
    # logical replication (here, via a row lock) must not be sent to the
    # subscriber. Otherwise the subscriber would receive a PREPARE with no
    # preceding BEGIN PREPARE and error out, breaking replication.
    for finish in ("COMMIT PREPARED", "ROLLBACK PREPARED"):
        publisher.sql_batch(
            "BEGIN",
            "SELECT a FROM tab_full WHERE a = 1 FOR SHARE",
            "PREPARE TRANSACTION 'test_empty_prepared'",
            f"{finish} 'test_empty_prepared'",
        )
    ...
    publisher.sql("INSERT INTO tab_full VALUES (31)")
    publisher.wait_for_catchup("tap_sub")

    assert subscriber.sql("SELECT count(*) FROM pg_prepared_xacts") == 0, (
        "empty prepared transaction is not replicated"
    )
    assert subscriber.sql("SELECT count(*) FROM tab_full WHERE a = 31") == 1, (
