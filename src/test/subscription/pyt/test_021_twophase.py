...
def test_twophase(create_pg):
    # Long test with many sequential waits and crash restarts; give each node a
    # fresh full timeout per poll rather than a single shared per-test deadline.
    ...
    publisher.sql(
        "BEGIN; INSERT INTO tab_full SELECT generate_series(1,10); "
    ...
    publisher.sql("BEGIN; INSERT INTO tab_full VALUES (11); PREPARE TRANSACTION 'test_prepared_tab_full'")

    # max_prepared_transactions = 0 on the subscriber makes the apply error.
    subscriber.wait_for_log(r"ERROR: ( [A-Z0-9]+:)? prepared transactions are disabled", log_location)
    ...
    publisher.sql("BEGIN; INSERT INTO tab_full VALUES (12); PREPARE TRANSACTION 'test_prepared_tab_full'")
    publisher.wait_for_catchup("tap_sub")
    assert subscriber.sql("SELECT count(*) FROM pg_prepared_xacts") == 1, (
        "transaction is prepared on subscriber"
    )
    ...
    publisher.sql(
        "BEGIN; INSERT INTO tab_full VALUES (12); INSERT INTO tab_full VALUES (13); "
    ...
    publisher.sql(
        "BEGIN; INSERT INTO tab_full VALUES (14); INSERT INTO tab_full VALUES (15); "
        "PREPARE TRANSACTION 'test_prepared_tab'"
    )
    subscriber.stop(mode="immediate")
    subscriber.start()
    ...
    publisher.sql(
        "BEGIN; INSERT INTO tab_full VALUES (16); INSERT INTO tab_full VALUES (17); "
        "PREPARE TRANSACTION 'test_prepared_tab'"
    )
    publisher.stop(mode="immediate")
    publisher.start()
    ...
    # ===== empty GID ========================================================
    publisher.sql("BEGIN; INSERT INTO tab_full VALUES (51); PREPARE TRANSACTION ''")
    publisher.wait_for_catchup("tap_sub")
    assert subscriber.sql("SELECT count(*) FROM pg_prepared_xacts") == 1, (
        "transaction is prepared on subscriber"
    )
    ...
    assert subscriber.sql("SELECT count(*) FROM tab_copy") == 1, "initial data in subscriber table"
    ...
    publisher.sql("BEGIN; INSERT INTO tab_copy VALUES (99); PREPARE TRANSACTION 'mygid'")
    ...
    assert publisher.sql("SELECT count(*) FROM tab_copy") == 6, "publisher inserted data"
    publisher.wait_for_catchup("appname_copy")
    publisher.wait_for_catchup("tap_sub")
    assert subscriber.sql("SELECT count(*) FROM pg_prepared_xacts") == 0, (
        "should be no prepared transactions on subscriber"
    )
    assert subscriber.sql("SELECT count(*) FROM tab_copy") == 2, "replicated data in subscriber table"
    ...
    ) == "d", "two-phase subscription option should be disabled"
    assert publisher.sql(
        "SELECT two_phase FROM pg_replication_slots WHERE slot_name = 'tap_sub_copy'"
    ) is False, "two-phase slot option should be disabled"

    # A prepare while two_phase is disabled is not replicated.
    publisher.sql("BEGIN; INSERT INTO tab_copy VALUES (100); PREPARE TRANSACTION 'newgid'")
    publisher.wait_for_catchup("appname_copy")
    assert subscriber.sql("SELECT count(*) FROM pg_prepared_xacts") == 0, (
        "should be no prepared transactions on subscriber"
    )
    ...
    subscriber.sql("ALTER SUBSCRIPTION tap_sub_copy SET (two_phase = true, failover = true)")
    subscriber.sql("ALTER SUBSCRIPTION tap_sub_copy ENABLE")

    publisher.sql("COMMIT PREPARED 'newgid'")
    publisher.wait_for_catchup("appname_copy")
    assert subscriber.sql("SELECT count(*) FROM tab_copy") == 3, "replicated data in subscriber table"
    assert subscriber.sql(
        "SELECT subtwophasestate FROM pg_subscription WHERE subname = 'tap_sub_copy'"
    ) == "e", "two-phase should be enabled"
