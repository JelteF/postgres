...
def test_twophase(create_pg):
    ...
    with publisher.connect() as pub:
        pub.sql("CREATE TABLE tab_copy (a int PRIMARY KEY)")
        pub.sql("INSERT INTO tab_copy SELECT generate_series(1,5)")
    with subscriber.connect() as sub:
        sub.sql("CREATE TABLE tab_copy (a int PRIMARY KEY)")
        sub.sql("INSERT INTO tab_copy VALUES (88)")
        assert sub.sql("SELECT count(*) FROM tab_copy") == 1, "initial data in subscriber table"
