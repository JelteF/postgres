...
def test_truncate(create_pg):
    ...
    with publisher.connect() as pub, subscriber.connect() as sub:
        for tab in ("tab1", "tab2", "tab3"):
            pub.sql(f"CREATE TABLE {tab} (a int PRIMARY KEY)")
            sub.sql(f"CREATE TABLE {tab} (a int PRIMARY KEY)")
        pub.sql("CREATE TABLE tab4 (x int PRIMARY KEY, y int REFERENCES tab3)")
        sub.sql("CREATE TABLE tab4 (x int PRIMARY KEY, y int REFERENCES tab3)")

        sub.sql("CREATE SEQUENCE seq1 OWNED BY tab1.a")
        sub.sql("ALTER SEQUENCE seq1 START 101")

        pub.sql("CREATE PUBLICATION pub1 FOR TABLE tab1")
        pub.sql("CREATE PUBLICATION pub2 FOR TABLE tab2 WITH (publish = insert)")
        pub.sql("CREATE PUBLICATION pub3 FOR TABLE tab3, tab4")
        sub.sql(f"CREATE SUBSCRIPTION sub1 CONNECTION '{connstr}' PUBLICATION pub1")
        sub.sql(f"CREATE SUBSCRIPTION sub2 CONNECTION '{connstr}' PUBLICATION pub2")
        sub.sql(f"CREATE SUBSCRIPTION sub3 CONNECTION '{connstr}' PUBLICATION pub3")
    ...
    publisher.wait_for_catchup("sub1")
    assert subscriber.sql("SELECT count(*), min(a), max(a) FROM tab1") == (0, None, None), (
        "truncate replicated"
    )
    ...
    publisher.wait_for_catchup("sub1")
    assert subscriber.sql("SELECT nextval('seq1')") == 101, "truncate restarted identities"
    ...
    publisher.sql("TRUNCATE tab2")
    publisher.wait_for_catchup("sub2")
    assert subscriber.sql("SELECT count(*), min(a), max(a) FROM tab2") == (0, None, None), (
        "truncate replicated after publication change"
    )

    # Multiple tables connected by foreign keys.
    with subscriber.connect() as sub:
        sub.sql("INSERT INTO tab3 VALUES (1), (2), (3)")
        sub.sql("INSERT INTO tab4 VALUES (11, 1), (111, 1), (22, 2)")
    publisher.sql("TRUNCATE tab3, tab4")
    publisher.wait_for_catchup("sub3")
    assert subscriber.sql("SELECT count(*), min(a), max(a) FROM tab3") == (0, None, None), (
        "truncate of multiple tables replicated"
    )
    assert subscriber.sql("SELECT count(*), min(x), max(x) FROM tab4") == (0, None, None), (
        "truncate of multiple tables replicated"
    )
    ...
    with subscriber.connect() as sub:
        sub.sql("INSERT INTO tab1 VALUES (1), (2), (3)")
        sub.sql("INSERT INTO tab2 VALUES (1), (2), (3)")
    ...
    with subscriber.connect() as sub:
        sub.sql(f"CREATE SUBSCRIPTION sub5_1 CONNECTION '{connstr}' PUBLICATION pub5")
        sub.sql(f"CREATE SUBSCRIPTION sub5_2 CONNECTION '{connstr}' PUBLICATION pub5")
    subscriber.wait_for_subscription_sync()
    ...
    publisher.wait_for_catchup("sub5_1")
    publisher.wait_for_catchup("sub5_2")
    assert subscriber.sql("SELECT count(*), min(a), max(a) FROM tab5") == (0, None, None), (
