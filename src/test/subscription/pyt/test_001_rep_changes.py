...
def test_rep_changes(create_pg):
    # Long test with many sequential waits and restarts; give each node a fresh
    # full timeout per poll rather than a single shared per-test deadline.
    ...
    publisher.sql("CREATE INDEX idx_no_replidentity_index ON tab_no_replidentity_index(c1)")
    ...
    subscriber.sql("CREATE INDEX idx_no_replidentity_index ON tab_no_replidentity_index(c1)")
    ...
    assert subscriber.sql("SELECT count(*), min(a), max(a) FROM tab_rep") == (20, -20, -1), (
    ...
    assert subscriber.sql("SELECT count(*), min(a), max(a) FROM tab_include") == (20, -20, -1), (
    ...
    publisher.sql("CREATE PUBLICATION tap_pub_temp1 FOR TABLE temp1 WITH (publish = insert)")
    ...
    assert subscriber.sql("SELECT count(*), min(a), max(a) FROM tab_full") == (20, 1, 100), (
    ...
    assert subscriber.sql("SELECT count(*), min(a), max(a) FROM tab_ins") == (1152, 1, 1100), (
    ...
    assert subscriber.sql("SELECT count(*), min(a), max(a) FROM tab_full") == (19, 0, 100), (
