...
def test_rep_changes(create_pg):
    ...
    with publisher.connect() as pub:
        pub.sql(
            "CREATE FUNCTION public.pg_get_replica_identity_index(int) "
            "RETURNS regclass LANGUAGE sql AS 'SELECT 1/0'"  # shall not be called
        )
        pub.sql("CREATE TABLE tab_notrep AS SELECT generate_series(1,10) AS a")
        pub.sql("CREATE TABLE tab_ins AS SELECT generate_series(1,1002) AS a")
        pub.sql("CREATE TABLE tab_full AS SELECT generate_series(1,10) AS a")
        pub.sql("CREATE TABLE tab_full2 (x text)")
        pub.sql("INSERT INTO tab_full2 VALUES ('a'), ('b'), ('b')")
        pub.sql("CREATE TABLE tab_rep (a int primary key)")
        pub.sql("CREATE TABLE tab_mixed (a int primary key, b text, c numeric)")
        pub.sql("INSERT INTO tab_mixed (a, b, c) VALUES (1, 'foo', 1.1)")
        pub.sql(
            "CREATE TABLE tab_include (a int, b text, "
            "CONSTRAINT covering PRIMARY KEY(a) INCLUDE(b))"
        )
        pub.sql("CREATE TABLE tab_full_pk (a int primary key, b text)")
        pub.sql("ALTER TABLE tab_full_pk REPLICA IDENTITY FULL")
        # REPLICA IDENTITY NOTHING allows only INSERT changes.
        pub.sql("CREATE TABLE tab_nothing (a int)")
        pub.sql("ALTER TABLE tab_nothing REPLICA IDENTITY NOTHING")
        # Replicate changes without a replica identity index.
        pub.sql("CREATE TABLE tab_no_replidentity_index(c1 int)")
        pub.sql("CREATE INDEX idx_no_replidentity_index ON tab_no_replidentity_index(c1)")
        # Replicate changes without columns.
        pub.sql("CREATE TABLE tab_no_col()")
        pub.sql("INSERT INTO tab_no_col default VALUES")

    # --- structure on the subscriber -----------------------------------------
    with subscriber.connect() as sub:
        sub.sql("CREATE TABLE tab_notrep (a int)")
        sub.sql("CREATE TABLE tab_ins (a int)")
        sub.sql("CREATE TABLE tab_full (a int)")
        sub.sql("CREATE TABLE tab_full2 (x text)")
        sub.sql("CREATE TABLE tab_rep (a int primary key)")
        sub.sql("CREATE TABLE tab_full_pk (a int primary key, b text)")
        sub.sql("ALTER TABLE tab_full_pk REPLICA IDENTITY FULL")
        sub.sql("CREATE TABLE tab_nothing (a int)")
        ...
        sub.sql("CREATE TABLE tab_no_replidentity_index(c1 int)")
        sub.sql("CREATE INDEX idx_no_replidentity_index ON tab_no_replidentity_index(c1)")
        sub.sql("CREATE TABLE tab_no_col()")

    # --- set up logical replication ------------------------------------------
    connstr = publisher.connstr()
    with publisher.connect() as pub:
        pub.sql("CREATE PUBLICATION tap_pub")
        pub.sql("CREATE PUBLICATION tap_pub_ins_only WITH (publish = insert)")
        pub.sql(
        ...
        pub.sql("ALTER PUBLICATION tap_pub_ins_only ADD TABLE tab_ins")
    subscriber.sql(
    ...
    with publisher.connect() as pub:
        pub.sql("INSERT INTO tab_ins SELECT generate_series(1,50)")
        pub.sql("DELETE FROM tab_ins WHERE a > 20")
        pub.sql("UPDATE tab_ins SET a = -a")
        pub.sql("INSERT INTO tab_rep SELECT generate_series(1,50)")
        pub.sql("DELETE FROM tab_rep WHERE a > 20")
        pub.sql("UPDATE tab_rep SET a = -a")
        pub.sql("INSERT INTO tab_mixed VALUES (2, 'bar', 2.2)")
        pub.sql("INSERT INTO tab_full_pk VALUES (1, 'foo'), (2, 'baz')")
        pub.sql("INSERT INTO tab_nothing VALUES (generate_series(1,20))")
        pub.sql("INSERT INTO tab_include SELECT generate_series(1,50)")
        pub.sql("DELETE FROM tab_include WHERE a > 20")
        pub.sql("UPDATE tab_include SET a = -a")
        pub.sql("INSERT INTO tab_no_replidentity_index VALUES(1)")
        pub.sql("INSERT INTO tab_no_col default VALUES")
    publisher.wait_for_catchup("tap_sub")

    with subscriber.connect() as sub:
        assert sub.sql("SELECT count(*), min(a), max(a) FROM tab_ins") == (1052, 1, 1002), (
            "check replicated inserts on subscriber"
        )
        assert sub.sql("SELECT count(*), min(a), max(a) FROM tab_rep") == (20, -20, -1), (
            "check replicated changes on subscriber"
        )
        assert sub.sql("SELECT * FROM tab_mixed") == [
            ("local", Decimal("1.1"), "foo", 1),
            ("local", Decimal("2.2"), "bar", 2),
        ], "check replicated changes with different column order"
        assert sub.sql("SELECT count(*) FROM tab_nothing") == 20, (
            "check replicated changes with REPLICA IDENTITY NOTHING"
        )
        assert sub.sql("SELECT count(*), min(a), max(a) FROM tab_include") == (20, -20, -1), (
            "check replicated changes with primary key index with included columns"
        )
        assert sub.sql("SELECT c1 FROM tab_no_replidentity_index") == 1, (
            "value replicated to subscriber without replica identity index"
        )
        assert sub.sql("SELECT count(*) FROM tab_no_col") == 2, (
    ...
    assert subscriber.sql("SELECT count(*), min(a), max(a) FROM tab_ins") == (1052, 1, 1002), (
    ...
    with publisher.connect() as pub:
        pub.sql("CREATE TABLE temp1 (a int)")
        pub.sql("CREATE TABLE temp2 (a int)")
    with subscriber.connect() as sub:
        sub.sql("CREATE TABLE temp1 (a int)")
        sub.sql("CREATE TABLE temp2 (a int)")
    with publisher.connect() as pub:
        pub.sql("CREATE PUBLICATION tap_pub_temp1 FOR TABLE temp1 WITH (publish = insert)")
        pub.sql("CREATE PUBLICATION tap_pub_temp2 FOR TABLE temp2")
    subscriber.sql(
    ...
    with publisher.connect() as pub:
        pub.sql("DROP PUBLICATION tap_pub_temp1")
        pub.sql("DROP PUBLICATION tap_pub_temp2")
        pub.sql("DROP TABLE temp1")
        pub.sql("DROP TABLE temp2")
    with subscriber.connect() as sub:
        sub.sql("DROP TABLE temp1")
        sub.sql("DROP TABLE temp2")
    ...
    with publisher.connect() as pub:
        pub.sql("UPDATE tab_full SET a = a * a")
        pub.sql("UPDATE tab_full2 SET x = 'bb' WHERE x = 'b'")
        pub.sql("UPDATE tab_mixed SET b = 'baz' WHERE a = 1")
        pub.sql("UPDATE tab_full_pk SET b = 'bar' WHERE a = 1")
    publisher.wait_for_catchup("tap_sub")

    with subscriber.connect() as sub:
        assert sub.sql("SELECT count(*), min(a), max(a) FROM tab_full") == (20, 1, 100), (
            "update works with REPLICA IDENTITY FULL and duplicate tuples"
        )
        assert sub.sql("SELECT x FROM tab_full2 ORDER BY 1") == ["a", "bb", "bb"], (
            "update works with REPLICA IDENTITY FULL and text datums"
        )
        assert sub.sql("SELECT * FROM tab_mixed ORDER BY a") == [
            ("local", Decimal("1.1"), "baz", 1),
            ("local", Decimal("2.2"), "bar", 2),
        ], "update works with different column order and subscriber local values"
        assert sub.sql("SELECT * FROM tab_full_pk ORDER BY a") == [
            (1, "bar"),
            (2, "baz"),
        ], "update works with REPLICA IDENTITY FULL and a primary key"

        sub.sql("DELETE FROM tab_full_pk")
        sub.sql("DELETE FROM tab_full WHERE a = 25")
    ...
    with publisher.connect() as pub:
        pub.sql("UPDATE tab_full_pk SET b = 'quux' WHERE a = 1")
        pub.sql("UPDATE tab_full SET a = a + 1 WHERE a = 25")
        pub.sql("DELETE FROM tab_full_pk WHERE a = 2")
    publisher.wait_for_catchup("tap_sub")
    ...
    with publisher.connect() as pub:
        pub.sql("UPDATE tab_mixed SET b = 'bar', c = 2.2 WHERE a = 2")
        pub.sql("ALTER TABLE tab_mixed DROP COLUMN b")
        pub.sql("UPDATE tab_mixed SET c = 11.11 WHERE a = 1")
    publisher.wait_for_catchup("tap_sub")
    ...
    with publisher.connect() as pub:
        pub.sql("INSERT INTO tab_ins SELECT generate_series(1001,1100)")
        pub.sql("DELETE FROM tab_rep")
    ...
    with subscriber.connect() as sub:
        assert sub.sql("SELECT count(*), min(a), max(a) FROM tab_ins") == (1152, 1, 1100), (
    ...
    with publisher.connect() as pub:
        pub.sql("ALTER PUBLICATION tap_pub_ins_only SET (publish = 'insert, delete')")
        pub.sql("ALTER PUBLICATION tap_pub_ins_only ADD TABLE tab_full")
        pub.sql("DELETE FROM tab_ins WHERE a > 0")
    subscriber.sql("ALTER SUBSCRIPTION tap_sub REFRESH PUBLICATION WITH (copy_data = false)")
    ...
    with subscriber.connect() as sub:
        ...
        assert sub.sql("SELECT count(*), min(a), max(a) FROM tab_full") == (19, 0, 100), (
