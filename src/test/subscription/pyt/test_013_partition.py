...
def test_partition(create_pg):
    ...
    with subscriber2.connect() as sub2:
        sub2.sql(
        ...
        sub2.sql(f"CREATE SUBSCRIPTION sub2 CONNECTION '{connstr}' PUBLICATION pub_all")
    ...
    with publisher.connect() as pub:
        pub.sql("INSERT INTO tab1 VALUES (1)")
        pub.sql("INSERT INTO tab1_1 (a) VALUES (3)")
        pub.sql("INSERT INTO tab1_2 VALUES (5)")
        pub.sql("INSERT INTO tab1 VALUES (0)")
    ...
    assert subscriber2.sql("SELECT c, a FROM tab1_2 ORDER BY 1, 2") == ("sub2_tab1_2", 5), (
    ...
    assert subscriber2.sql("SELECT c, a FROM tab1_def ORDER BY 1, 2") == ("sub2_tab1_def", 0), (
        "inserts into tab1_def replicated"
    )

    # update (replicated as update)
    with publisher.connect() as pub:
        pub.sql("UPDATE tab1 SET a = 2 WHERE a = 1")
        ...
        pub.sql("UPDATE tab1 SET a = 6 WHERE a = 5")
        pub.sql("UPDATE tab1 SET a = 4 WHERE a = 6")
        pub.sql("UPDATE tab1 SET a = 6 WHERE a = 4")
    publisher.wait_for_catchup("sub1")
    publisher.wait_for_catchup("sub2")
    ...
    assert subscriber2.sql("SELECT c, a FROM tab1_2 ORDER BY 1, 2") == ("sub2_tab1_2", 6), (
        "tab1_2 updated"
    )
    assert subscriber2.sql(
        "SELECT * FROM sub2_trigger_activity ORDER BY tgtab, tgop, tgwhen, olda, newa"
    ...
    assert subscriber2.sql("SELECT c, a FROM tab1_def ORDER BY 1") == ("sub2_tab1_def", 0), (
        "tab1_def unchanged"
    )

    # update (replicated as delete+insert)
    with publisher.connect() as pub:
        pub.sql("UPDATE tab1 SET a = 1 WHERE a = 0")
        pub.sql("UPDATE tab1 SET a = 4 WHERE a = 1")
    publisher.wait_for_catchup("sub1")
    publisher.wait_for_catchup("sub2")
    ...
    # delete
    with publisher.connect() as pub:
        pub.sql("DELETE FROM tab1 WHERE a IN (2, 3, 5)")
        pub.sql("DELETE FROM tab1_2")
    publisher.wait_for_catchup("sub1")
    publisher.wait_for_catchup("sub2")

    assert subscriber1.sql("SELECT a FROM tab1") == [], "delete from tab1_1, tab1_2 replicated"
    assert subscriber2.sql("SELECT a FROM tab1_1") == [], "delete from tab1_1 replicated"
    assert subscriber2.sql("SELECT a FROM tab1_2") == [], "delete from tab1_2 replicated"
    ...
    assert subscriber2.sql("SELECT a FROM tab1_2 ORDER BY 1") == [], "truncate of tab1_2 replicated"

    publisher.sql("TRUNCATE tab1")
    publisher.wait_for_catchup("sub1")
    publisher.wait_for_catchup("sub2")

    assert subscriber1.sql("SELECT a FROM tab1 ORDER BY 1") == [], "truncate of tab1_1 replicated"
    assert subscriber2.sql("SELECT a FROM tab1 ORDER BY 1") == [], "truncate of tab1 replicated"
    ...
    # insert
    with publisher.connect() as pub:
        pub.sql("INSERT INTO tab1 VALUES (1), (0)")
        pub.sql("INSERT INTO tab1_1 (a) VALUES (3)")
        pub.sql("INSERT INTO tab1_2 VALUES (5)")
        pub.sql("INSERT INTO tab2 VALUES (0), (3), (5)")
        pub.sql("INSERT INTO tab3 VALUES (1), (0), (3), (5)")
        # Replicated through the partition root (FOR ALL TABLES partition).
        pub.sql("INSERT INTO tab4 VALUES (0)")
    ...
    assert subscriber2.sql("SELECT a FROM tab4 ORDER BY 1") == [-1, 0], "inserts into tab4 replicated"
    assert subscriber2.sql("SELECT a FROM tab4_1 ORDER BY 1") == [], "inserts into tab4_1 replicated"
    ...
    with publisher.connect() as pub:
        pub.sql("UPDATE tab1 SET a = 6 WHERE a = 5")
        pub.sql("UPDATE tab2 SET a = 6 WHERE a = 5")
        pub.sql("UPDATE tab3 SET a = 6 WHERE a = 5")
    publisher.wait_for_catchup("sub_viaroot")
    publisher.wait_for_catchup("sub2")
    ...
    # update (replicated as delete+insert)
    with publisher.connect() as pub:
        pub.sql("UPDATE tab1 SET a = 2 WHERE a = 6")
        pub.sql("UPDATE tab2 SET a = 2 WHERE a = 6")
        pub.sql("UPDATE tab3 SET a = 2 WHERE a = 6")
    publisher.wait_for_catchup("sub_viaroot")
    publisher.wait_for_catchup("sub2")
    ...
    # delete
    with publisher.connect() as pub:
        pub.sql("DELETE FROM tab1")
        pub.sql("DELETE FROM tab2")
        pub.sql("DELETE FROM tab3")
    publisher.wait_for_catchup("sub_viaroot")
    publisher.wait_for_catchup("sub2")
    ...
    # truncate
    with publisher.connect() as pub:
        pub.sql("INSERT INTO tab1 VALUES (1), (2), (5)")
        pub.sql("INSERT INTO tab2 VALUES (1), (2), (5)")
        # these will NOT be replicated (publish_via_partition_root)
        pub.sql("TRUNCATE tab1_2, tab2_1, tab3_1")
    publisher.wait_for_catchup("sub_viaroot")
    publisher.wait_for_catchup("sub2")
    ...
    assert subscriber2.sql("SELECT a FROM tab3_1") == [], "truncate of tab3_1 replicated"

    # check the leaf->root tuple conversion map is rebuilt when a column is added
    with publisher.connect() as pub:
        pub.sql("ALTER TABLE tab2 DROP b, ADD COLUMN c text DEFAULT 'pub_tab2', ADD b text")
        pub.sql("INSERT INTO tab2 (a, b) VALUES (1, 'xxx'), (3, 'yyy'), (5, 'zzz')")
        pub.sql("INSERT INTO tab2 (a, b, c) VALUES (6, 'aaa', 'xxx_c')")
    publisher.wait_for_catchup("sub_viaroot")
    publisher.wait_for_catchup("sub2")
    ...
    with publisher.connect() as pub:
        pub.sql("UPDATE tab2 SET b = 'quux' WHERE a = 5")
        pub.sql("DELETE FROM tab2 WHERE a = 1")
    publisher.wait_for_catchup("sub_viaroot")
    publisher.wait_for_catchup("sub2")
    ...
    with publisher.connect() as pub:
        pub.sql("INSERT INTO tab5 VALUES (1, 1)")
        pub.sql("UPDATE tab5 SET a = 2 WHERE a = 1")
    publisher.wait_for_catchup("sub2")
