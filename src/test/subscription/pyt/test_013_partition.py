...
def test_partition(create_pg):
    # Long test with many sequential waits across three nodes; give each a
    ...
    subscriber2.sql(f"CREATE SUBSCRIPTION sub2 CONNECTION '{connstr}' PUBLICATION pub_all")
    subscriber2.sql(
        """
    ...
    publisher.sql("ALTER TABLE tab2 DROP b, ADD COLUMN c text DEFAULT 'pub_tab2', ADD b text")
    ...
    subscriber1.append_conf("track_commit_timestamp = on")
    ...
    subscriber1.append_conf("track_commit_timestamp = off")
    subscriber1.pg_ctl("restart")
