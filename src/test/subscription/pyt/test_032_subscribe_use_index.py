...
def test_subscribe_use_index(create_pg):
    ...
    publisher.sql("UPDATE test_replica_id_full SET x = 100, y = '200' WHERE x IN (1, 2)")
    ...
    publisher.sql("UPDATE people SET firstname = 'no-name' WHERE firstname = 'first_name_1'")
    publisher.sql(
    ...
    assert subscriber.sql("SELECT count(*) FROM people WHERE firstname = 'no-name'") == 0, (
        "ensure subscriber has the correct data at the end of the test"
    )
