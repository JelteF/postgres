...
def test_subscribe_use_index(create_pg):
    ...
    with publisher.connect() as pub:
        pub.sql("CREATE TABLE test_replica_id_full (x int, y text)")
        pub.sql("ALTER TABLE test_replica_id_full REPLICA IDENTITY FULL")
    with subscriber.connect() as sub:
        sub.sql("CREATE TABLE test_replica_id_full (x int, y text)")
        sub.sql("CREATE INDEX test_replica_id_full_idx ON test_replica_id_full(x,y)")

    with publisher.connect() as pub:
        pub.sql(
            "INSERT INTO test_replica_id_full SELECT (i%10), (i%10)::text FROM generate_series(0,10) i"
        )
        pub.sql("CREATE PUBLICATION tap_pub_rep_full FOR TABLE test_replica_id_full")
    subscriber.sql(
    ...
    with publisher.connect() as pub:
        pub.sql("DELETE FROM test_replica_id_full WHERE x IN (5, 6)")
        pub.sql("UPDATE test_replica_id_full SET x = 100, y = '200' WHERE x IN (1, 2)")
    ...
    with subscriber.connect() as sub:
        ...
        ) == 2, "ensure subscriber has the correct data at the end of the test"
        assert sub.sql(
            "select count(*) from test_replica_id_full where x in (5, 6)"
        ) == 0, "ensure subscriber has the correct data at the end of the test"

    with publisher.connect() as pub:
        pub.sql("DROP PUBLICATION tap_pub_rep_full")
        pub.sql("DROP TABLE test_replica_id_full")
    with subscriber.connect() as sub:
        sub.sql("DROP SUBSCRIPTION tap_sub_rep_full")
        sub.sql("DROP TABLE test_replica_id_full")
    ...
    with publisher.connect() as pub:
        ...
        pub.sql("CREATE PUBLICATION tap_pub_rep_full FOR TABLE users_table_part")
    subscriber.sql(
        f"CREATE SUBSCRIPTION tap_sub_rep_full CONNECTION '{connstr}' "
        "PUBLICATION tap_pub_rep_full"
    )
    ...
    with publisher.connect() as pub:
        pub.sql("UPDATE users_table_part SET value_1 = 0 WHERE user_id = 4")
        pub.sql("DELETE FROM users_table_part WHERE user_id = 1 and value_1 = 1")
        pub.sql("DELETE FROM users_table_part WHERE user_id = 12 and value_1 = 12")
    ...
    with subscriber.connect() as sub:
        ...
        ) == 10907, "ensure subscriber has the correct data at the end of the test"
        assert sub.sql(
            "select count(DISTINCT(user_id,value_1, value_2)) from users_table_part"
        ) == 99, "ensure subscriber has the correct data at the end of the test"

    with publisher.connect() as pub:
        pub.sql("DROP PUBLICATION tap_pub_rep_full")
        pub.sql("DROP TABLE users_table_part")
    with subscriber.connect() as sub:
        sub.sql("DROP SUBSCRIPTION tap_sub_rep_full")
        sub.sql("DROP TABLE users_table_part")

    # =====================================================================
    # Subscription will not use expression-only or partial indexes.
    with publisher.connect() as pub:
        pub.sql("CREATE TABLE people (firstname text, lastname text)")
        pub.sql("ALTER TABLE people REPLICA IDENTITY FULL")
    with subscriber.connect() as sub:
        sub.sql("CREATE TABLE people (firstname text, lastname text)")
        sub.sql(
    ...
    with publisher.connect() as pub:
        ...
        pub.sql("CREATE PUBLICATION tap_pub_rep_full FOR TABLE people")
    subscriber.sql(
        f"CREATE SUBSCRIPTION tap_sub_rep_full CONNECTION '{connstr}' "
        "PUBLICATION tap_pub_rep_full"
    )
    ...
    with publisher.connect() as pub:
        pub.sql("UPDATE people SET firstname = 'no-name' WHERE firstname = 'first_name_1'")
        pub.sql(
    ...
    with publisher.connect() as pub:
        pub.sql("DELETE FROM people WHERE firstname = 'first_name_3'")
        pub.sql(
    ...
    with subscriber.connect() as sub:
        ...
        assert sub.sql("SELECT count(*) FROM people") == 199, (
            "ensure subscriber has the correct data at the end of the test"
        )

    with publisher.connect() as pub:
        pub.sql("DROP PUBLICATION tap_pub_rep_full")
        pub.sql("DROP TABLE people")
    with subscriber.connect() as sub:
        sub.sql("DROP SUBSCRIPTION tap_sub_rep_full")
        sub.sql("DROP TABLE people")
    ...
    with publisher.connect() as pub:
        pub.sql("UPDATE people SET firstname = 'no-name' WHERE firstname = 'first_name_1'")
        pub.sql("DELETE FROM people WHERE firstname = 'no-name'")
    publisher.wait_for_catchup("tap_sub")
    subscriber.poll_query_until(
    ...
    with subscriber.connect() as sub:
        assert sub.sql("SELECT count(*) FROM people") == 20, (
            "ensure subscriber has the correct data at the end of the test"
        )
        assert sub.sql("SELECT count(*) FROM people WHERE firstname = 'no-name'") == 0, (
            "ensure subscriber has the correct data at the end of the test"
        )
    ...
    with publisher.connect() as pub:
        pub.sql("CREATE TABLE test_replica_id_full (x int)")
        pub.sql("ALTER TABLE test_replica_id_full REPLICA IDENTITY FULL")
    with subscriber.connect() as sub:
        sub.sql("CREATE TABLE test_replica_id_full (x int, y int)")
        sub.sql("CREATE INDEX test_replica_id_full_idx ON test_replica_id_full(x,y)")
    ...
    with publisher.connect() as pub:
        pub.sql("INSERT INTO test_replica_id_full VALUES (1), (2), (3)")
        pub.sql("UPDATE test_replica_id_full SET x = x + 1 WHERE x = 1")
    publisher.wait_for_catchup("tap_sub")
    subscriber.poll_query_until(
    ...
    with subscriber.connect() as sub:
        ...
        ) == 7, "ensure subscriber has the correct data at the end of the test"
        assert sub.sql(
            "select count(*) from test_replica_id_full WHERE y IS NULL"
        ) == 3, "ensure subscriber has the correct data at the end of the test"
    ...
    with publisher.connect() as pub:
        pub.sql("CREATE TABLE test_replica_id_full (x int, y int)")
        pub.sql("ALTER TABLE test_replica_id_full REPLICA IDENTITY FULL")
    with subscriber.connect() as sub:
        sub.sql("CREATE TABLE test_replica_id_full (x int, y int)")
        sub.sql(
    ...
    ) == 1, "ensure subscriber has the correct data at the end of the test"
