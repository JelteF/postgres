...
def test_run_as_table_owner(create_pg):
    ...
    def expect_replication(tbl, cnt, mn, mx, name):
        ...
        assert subscriber.sql(f"SELECT COUNT(i), MIN(i), MAX(i) FROM {tbl}") == (cnt, mn, mx), name
    ...
    expect_failure("alice.unpartitioned", 2, 5, 7, "with no privileges cannot replicate")
    ...
    expect_failure(
        "alice.unpartitioned", 3, 5, 9, "without SELECT privilege cannot replicate UPDATE/DELETE"
    )
    ...
    expect_failure("alice.unpartitioned", 2, 7, 11, "with SET ROLE but not INHERIT cannot replicate")

    # INHERIT (not SET ROLE) works.
    subscriber.sql("GRANT regress_alice TO regress_admin WITH INHERIT TRUE, SET FALSE")
    expect_replication("alice.unpartitioned", 3, 7, 13, "with INHERIT but not SET ROLE can replicate")
    ...
    expect_failure("alice.unpartitioned", 3, 7, 13, "with no privileges cannot replicate")

    # run_as_owner = false runs apply as the table owner.
    subscriber.sql("ALTER SUBSCRIPTION admin_sub SET (run_as_owner = false)")
    expect_replication(
        "alice.unpartitioned", 4, 7, 14, "can replicate after setting run_as_owner to false"
    )

    with subscriber.connect() as sub:
        sub.sql("DROP SUBSCRIPTION admin_sub")
        sub.sql("TRUNCATE alice.unpartitioned")
    ...
    with subscriber.connect() as sub:
        sub.sql("ALTER ROLE regress_admin2 NOSUPERUSER")
        sub.sql("GRANT regress_alice TO regress_admin2 WITH INHERIT FALSE, SET TRUE")
        sub.sql("ALTER SUBSCRIPTION admin_sub ENABLE")
    subscriber.wait_for_subscription_sync(publisher, "admin_sub")
    expect_replication("alice.unpartitioned", 4, 7, 14, "table owner can do the initial data copy")
