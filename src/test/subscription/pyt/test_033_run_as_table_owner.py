...
def test_run_as_table_owner(create_pg):
    ...
    def publish_insert(tbl, new_i):
        publisher.sql(
            f"SET SESSION AUTHORIZATION regress_alice; INSERT INTO {tbl} (i) VALUES ({new_i})"
        )

    def publish_update(tbl, old_i, new_i):
        publisher.sql(
            f"SET SESSION AUTHORIZATION regress_alice; "
            f"UPDATE {tbl} SET i = {new_i} WHERE i = {old_i}"
        )

    def publish_delete(tbl, old_i):
        publisher.sql(
            f"SET SESSION AUTHORIZATION regress_alice; DELETE FROM {tbl} WHERE i = {old_i}"
        )
