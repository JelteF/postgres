...
def test_twophase_stream(create_pg):
    ...
    def test_streaming(appname, is_parallel):
        ...
        publisher.sql("ROLLBACK PREPARED 'test_prepared_tab'")
        publisher.wait_for_catchup(appname)
        assert subscriber.sql("SELECT count(*), count(c), count(d = 999) FROM test_tab") == (3, 3, 3), (
        ...
        publisher.sql("COMMIT PREPARED 'test_prepared_tab'")
        publisher.wait_for_catchup(appname)
        assert subscriber.sql("SELECT count(*), count(c), count(d = 999) FROM test_tab") == (5, 5, 5), (
    ...
    offset = subscriber.current_log_position()
    publisher.sql("BEGIN; INSERT INTO test_tab_2 values(1); PREPARE TRANSACTION 'xact'")
    subscriber.wait_for_log(
    ...
    publisher.sql("BEGIN; INSERT INTO test_tab_2 values(2); PREPARE TRANSACTION 'xact'")
    publisher.sql("COMMIT PREPARED 'xact'")
