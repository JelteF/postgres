...
def test_twophase_stream(create_pg):
    ...
    def test_streaming(appname, is_parallel):
        ...
        with subscriber.connect() as sub:
            assert sub.sql("SELECT count(*), count(c), count(d = 999) FROM test_tab") == (4, 4, 4), (
                "Rows inserted by 2PC have committed on subscriber, and extra columns contain local defaults"
            )
            assert sub.sql("SELECT count(*) FROM pg_prepared_xacts") == 0, (
        ...
        publisher.wait_for_catchup(appname)
        with subscriber.connect() as sub:
            assert sub.sql("SELECT count(*), count(c), count(d = 999) FROM test_tab") == (2, 2, 2), (
        ...
        publisher.sql("COMMIT PREPARED 'test_prepared_tab'")
        publisher.wait_for_catchup(appname)
        assert subscriber.sql("SELECT count(*), count(c), count(d = 999) FROM test_tab") == (4, 4, 4), (
            "Rows inserted by 2PC have committed on subscriber, and extra columns contain local defaults"
        )
        ...
        with publisher.connect() as pub:
            pub.sql("INSERT INTO test_tab VALUES (99999, 'foobar')")
            pub.sql("ROLLBACK PREPARED 'test_prepared_tab'")
        publisher.wait_for_catchup(appname)
        with subscriber.connect() as sub:
            assert sub.sql("SELECT count(*), count(c), count(d = 999) FROM test_tab") == (3, 3, 3), (
        ...
        with publisher.connect() as pub:
            pub.sql("INSERT INTO test_tab VALUES (99999, 'foobar')")
            pub.sql("COMMIT PREPARED 'test_prepared_tab'")
        publisher.wait_for_catchup(appname)
        with subscriber.connect() as sub:
            assert sub.sql("SELECT count(*), count(c), count(d = 999) FROM test_tab") == (5, 5, 5), (
    ...
    # --- setup ---------------------------------------------------------------
    with publisher.connect() as pub:
        pub.sql("CREATE TABLE test_tab (a int primary key, b bytea)")
        pub.sql("INSERT INTO test_tab VALUES (1, 'foo'), (2, 'bar')")
        pub.sql("CREATE TABLE test_tab_2 (a int)")
    with subscriber.connect() as sub:
        sub.sql(
            "CREATE TABLE test_tab (a int primary key, b bytea, "
            "c timestamptz DEFAULT now(), d bigint DEFAULT 999)"
        )
        sub.sql("CREATE TABLE test_tab_2 (a int)")
    ...
    assert subscriber.sql("SELECT count(*), count(c), count(d = 999) FROM test_tab") == (2, 2, 2), (
    ...
    offset = subscriber.current_log_position()
    subscriber.wait_for_log(r"ERROR: ( [A-Z0-9]+:)? prepared transactions are disabled", offset)
    ...
    subscriber.wait_for_log(r"ERROR: .*logical replication parallel apply worker.*", offset)
    ...
    with subscriber.connect() as sub:
        assert sub.sql("SELECT count(*) FROM pg_subscription_rel") == 0, (
            "check subscription relation status was dropped on subscriber"
        )
        assert sub.sql("SELECT count(*) FROM pg_replication_origin") == 0, (
