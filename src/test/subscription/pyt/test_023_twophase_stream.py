...
def test_twophase_stream(create_pg):
    ...
    subscriber.sql_oneshot("SELECT 1")  # ensure the reload took effect
    ...
    # Capture the log position before the publisher work: the apply worker can
    # hit the error before we get around to reading the offset otherwise.
    offset = subscriber.current_log_position()
