...
Tests that more complex datatypes (arrays, enums, composites, nested
composites/arrays, ranges, hstore, and a domain with a SQL-language constraint)
are replicated correctly by logical replication, across initial copy and
batches of INSERT/UPDATE/DELETE.

The subscriber data is checked by capturing psql's tuples-only text output for a
batch of SELECTs (matching the original TAP test) and comparing it verbatim,
since the object model would collapse these many differently-shaped result sets
and lose the exact array/composite text formatting being verified.
"""

# A batch of SELECTs whose concatenated psql -At output is compared verbatim.
SELECT_BATCH = """
    SET timezone = '+2';
...
EXPECTED_AFTER_INSERT = r'''1|{1,2,3}
2|{2,3,1}
3|{3,2,1}
4|{4,3,2}
5|{5,NULL,3}
...
EXPECTED_AFTER_UPDATE = r'''1|{4,5,6}
2|{2,3,1}
3|{3,2,1}
4|{4,5,6,1}
5|{4,5,6,1}
...
EXPECTED_AFTER_DELETE = r'''3|{3,2,1}
4|{4,5,6,1}
5|{4,5,6,1}
{3,1,2}|{c,a,b}|{3.3,1.1,2.2}|{"3 years","1 year","2 years"}
{4,1,2}|{c,d,e}|{3,4,5}|{"3 days 00:00:01","4 days 00:00:02","5 days 00:00:03"}
...
# Table/type setup, run on both publisher and subscriber.
DDL = """
...
def test_types(create_pg, pg_bin):
    ...
    def check(expected, testname):
        result = pg_bin.run(
            "psql", "-XAtq", "-f", "-", server=subscriber, input=SELECT_BATCH, check=True
        )
    ...
    publisher.sql(
        ...
        INSERT INTO tst_one_enum (a, b) VALUES
            (1, 'a'), (2, 'b'), (3, 'c'), (4, 'd'), (5, NULL);
        INSERT INTO tst_enums (a, b) VALUES
            ('a', '{b, c}'), ('b', '{c, a}'), ('c', '{b, a}'), ('d', '{c, b}'), ('e', '{d, NULL}');
        INSERT INTO tst_one_comp (a, b) VALUES
            (1, ROW(1.0, 'a', 1)), (2, ROW(2.0, 'b', 2)), (3, ROW(3.0, 'c', 3)),
            (4, ROW(4.0, 'd', 4)), (5, ROW(NULL, NULL, 5));
        ...
        INSERT INTO tst_comp_enum (a, b) VALUES
            (1, ROW(1.0, 'a', 1)), (2, ROW(2.0, 'b', 2)), (3, ROW(3.0, 'c', 3)),
            (4, ROW(4.0, 'd', 4)), (5, ROW(NULL, 'e', NULL));
        ...
        INSERT INTO tst_comp_one_enum_array (a, b) VALUES
            (1, ROW(1.0, '{a, b, c}', 1)), (2, ROW(2.0, '{a, b, c}', 2)),
            (3, ROW(3.0, '{a, b, c}', 3)), (4, ROW(4.0, '{c, b, d}', 4)),
        ...
        INSERT INTO tst_hstore (a, b) VALUES
            (1, '"a"=>"1"'), (2, '"zzz"=>"foo"'), (3, '"123"=>"321"'), (4, '"yellow horse"=>"moaned"');
        INSERT INTO tst_dom_constr VALUES (10);
        """
    )
    publisher.wait_for_catchup("tap_sub")
    check(EXPECTED_AFTER_INSERT, "check replicated inserts on subscriber")
    ...
    publisher.wait_for_catchup("tap_sub")
    check(EXPECTED_AFTER_UPDATE, "check replicated updates on subscriber")
    ...
    publisher.wait_for_catchup("tap_sub")
    check(EXPECTED_AFTER_DELETE, "check replicated deletes on subscriber")

    # A domain whose SQL-language constraint needs an active snapshot.
    publisher.sql("INSERT INTO tst_dom_constr VALUES (11)")
    publisher.wait_for_catchup("tap_sub")
    assert subscriber.sql("SELECT sum(a) FROM tst_dom_constr") == 21, (
