...
def test_sequences(create_pg):
    ...
    assert subscriber.sql("SELECT last_value, is_called FROM regress_s1") == (100, True), (
        "initial test data replicated"
    )
    assert subscriber.sql(f"SELECT last_value, is_called FROM {QUOTE}") == (100, True), (
    ...
    subscriber.poll_query_until(SYNCED)
    assert publisher.sql("SELECT last_value, is_called FROM regress_s1") == (200, True), (
        "Check sequence value in the publisher"
    )
    assert subscriber.sql("SELECT last_value, is_called FROM regress_s1") == (100, True), (
        "REFRESH PUBLICATION will not sync existing sequence"
    )
    assert subscriber.sql("SELECT last_value, is_called FROM regress_s2") == (100, True), (
    ...
    subscriber.poll_query_until(SYNCED)
    assert subscriber.sql("SELECT last_value, is_called FROM regress_s1") == (200, True), (
        "REFRESH SEQUENCES will sync existing sequences"
    )
    assert subscriber.sql("SELECT last_value, is_called FROM regress_s2") == (200, True), (
        "REFRESH SEQUENCES will sync existing sequences"
    )
    assert subscriber.sql("SELECT last_value, is_called FROM regress_s3") == (1, False), (
    ...
    # Insufficient privileges on a sequence only warn and let replication retry.
    publisher.sql(
        """
