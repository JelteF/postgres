...
def test_sequences(create_pg):
    ...
    with subscriber.connect() as sub:
        assert sub.sql("SELECT last_value, is_called FROM regress_s1") == (100, True), (
            "initial test data replicated"
        )
        assert sub.sql(f"SELECT last_value, is_called FROM {QUOTE}") == (100, True), (
    ...
    with subscriber.connect() as sub:
        ...
        assert sub.sql("SELECT last_value, is_called FROM regress_s2") == (100, True), (
    ...
    subscriber.poll_query_until(SYNCED)
    with subscriber.connect() as sub:
        assert sub.sql("SELECT last_value, is_called FROM regress_s1") == (200, True), (
            "REFRESH SEQUENCES will sync existing sequences"
        )
        assert sub.sql("SELECT last_value, is_called FROM regress_s2") == (200, True), (
            "REFRESH SEQUENCES will sync existing sequences"
        )
        assert sub.sql("SELECT last_value, is_called FROM regress_s3") == (1, False), (
