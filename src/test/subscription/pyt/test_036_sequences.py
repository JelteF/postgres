...
def test_sequences(create_pg):
    ...
    # REFRESH SEQUENCES must still be able to update sequence values and mark
    # the sequence ready when default_transaction_read_only is enabled on the
    # subscriber, i.e. the sequencesync worker must not inherit it.
    subscriber.append_conf(default_transaction_read_only=True)
    subscriber.reload()

    publisher.sql(
        "INSERT INTO regress_seq_test SELECT nextval('regress_s3') FROM generate_series(1,100)"
    )
    # The REFRESH itself is a write, so turn the setting off around it while the
    # worker it spawns still starts up with the setting enabled.
    subscriber.sql_batch(
    ...
    assert subscriber.sql("SELECT last_value, is_called FROM regress_s3") == (
        200,
        True,
    ), "REFRESH SEQUENCES updates the value with default_transaction_read_only on"
    assert (
        subscriber.sql(
            "SELECT srsubstate FROM pg_subscription_rel "
            "WHERE srrelid = 'regress_s3'::regclass"
        )
        == "r"
    ), "sequence is marked ready after REFRESH SEQUENCES with read-only default"

    subscriber.adjust_conf(default_transaction_read_only=None)
    subscriber.reload()
