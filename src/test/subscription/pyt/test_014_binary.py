...
def test_binary(create_pg):
    ...
    publisher = create_pg("publisher", allows_streaming="logical", conf={"log_statement": "all"})
    ...
    publisher.wait_for_log(
        r"LOG: ( [A-Z0-9]+:)? statement: COPY (.+)? TO STDOUT WITH \(FORMAT binary\)", offset
    )
    ...
    assert numerical() == [
        ("1", "42", None, "10"), ("2", "42", None, "20"),
        ("3", "42", None, "30"), ("4", "42", None, "40"),
    ], "check updated replicated data on subscriber"

    # Switch to text format and back to binary.
    subscriber.sql("ALTER SUBSCRIPTION tsub SET (binary = false)")
    publisher.sql("INSERT INTO public.test_numerical (a, b, c, d) VALUES (5, 5.2, 5.3, 50)")
    publisher.wait_for_catchup("tsub")
    assert numerical() == [
        ("1", "42", None, "10"), ("2", "42", None, "20"),
        ("3", "42", None, "30"), ("4", "42", None, "40"),
    ...
    with subscriber.connect() as sub:
        sub.sql("CREATE TABLE public.test_mismatching_types (a int PRIMARY KEY)")
        sub.sql("ALTER SUBSCRIPTION tsub REFRESH PUBLICATION")
    subscriber.wait_for_log(
    ...
    offset = publisher.current_log_position()
    subscriber.sql("ALTER SUBSCRIPTION tsub SET (binary = false)")
    publisher.wait_for_log(r"LOG: ( [A-Z0-9]+:)? statement: COPY (.+)? TO STDOUT\n", offset)
    subscriber.wait_for_subscription_sync(publisher, "tsub")
    assert subscriber.sql("SELECT a FROM test_mismatching_types ORDER BY a") == [1, 2], (
