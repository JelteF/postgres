...
def test_origin(create_pg):
    ...
    with node_A.connect() as a:
        a.sql("DROP TABLE tab_new")
        a.sql(f"DROP SUBSCRIPTION {SUBNAME_AB2}")
        a.sql(f"DROP SUBSCRIPTION {SUBNAME_AB}")
        a.sql("DROP PUBLICATION tap_pub_A")
    with node_B.connect() as b:
        b.sql("DROP TABLE tab_new")
        b.sql(f"DROP SUBSCRIPTION {SUBNAME_BA}")
        b.sql("DROP PUBLICATION tap_pub_B")
