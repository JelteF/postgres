...
def test_crash_temp_files(create_pg):
    ...
    setup = node.connect()
    setup.sql("ALTER SYSTEM SET remove_temp_files_after_crash = on")
    setup.sql("ALTER SYSTEM SET log_connections = receipt")
    setup.sql("ALTER SYSTEM SET work_mem = '64kB'")
    setup.sql("ALTER SYSTEM SET restart_after_crash = on")
    setup.sql("SELECT pg_reload_conf()")

    setup.sql("CREATE TABLE tab_crash (a integer UNIQUE)")

    def crash_with_temp_file():
        killme = node.background()
        killme2 = node.background()
