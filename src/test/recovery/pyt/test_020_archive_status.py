...
def test_archive_status(create_pg):
    ...
    with primary.connect() as c:
        segment_1 = c.sql("SELECT pg_walfile_name(pg_current_wal_lsn())")
        ...
        c.sql("CREATE TABLE mine AS SELECT generate_series(1,10) AS x")
        c.sql("SELECT pg_switch_wal()")
        c.sql("CHECKPOINT")
    ...
    with primary.connect() as c:
        segment_2 = c.sql("SELECT pg_walfile_name(pg_current_wal_lsn())")
        seg2_ready_name = f"{segment_2}.ready"
        seg2_done_name = f"{segment_2}.done"
        c.sql("INSERT INTO mine SELECT generate_series(10,20) AS x")
        c.sql("CHECKPOINT")
        primary_lsn = c.sql("SELECT pg_switch_wal()")
    ...
    assert not (standby1_data / "pg_wal/archive_status" / f"{segment_1}.ready").is_file(), (
    ...
    standby2 = create_pg(
        "standby2", from_backup=backup, restoring=primary, conf={"archive_mode": "always"}
    )
