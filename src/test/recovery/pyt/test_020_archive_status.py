...
def test_archive_status(create_pg):
    primary = create_pg(
        "primary", archiving=True, allows_streaming=True, conf=["autovacuum = off"]
    )
    ...
    primary.sql("SELECT pg_reload_conf()")
    primary.poll_query_until("SELECT archived_count FROM pg_stat_archiver", 1)
    ...
    primary.poll_query_until(
        "SELECT last_archived_wal FROM pg_stat_archiver", segment_2
    )
    ...
    primary.sql("SELECT pg_backup_start('onebackup'); SELECT pg_backup_stop()")
