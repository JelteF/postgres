...
def _wal_receiver_present(node, present):
    node.poll_query_until(
        "SELECT EXISTS (SELECT * FROM pg_stat_wal_receiver)", present, dbname=TEST_DB
    )
...
def test_vacuum_horizon_floor(create_pg):
    ...
    session_a = primary.background(dbname=TEST_DB)
    session_b = primary.background(dbname=TEST_DB)
    ...
    primary.sql(
        ...
        INSERT INTO {TABLE} SELECT generate_series(1, {NROWS}) % 3;
    ...
    replica.sql("ALTER SYSTEM SET primary_conninfo = ''", dbname=TEST_DB)
    replica.sql("SELECT pg_reload_conf()", dbname=TEST_DB)
    ...
    res = session_b.sql(
        ...
        DECLARE {CURSOR} CURSOR FOR SELECT * FROM {TABLE} WHERE col1 = 7;
        FETCH {CURSOR};
        """
    )
    ...
    vacuum = session_a.asql(f"VACUUM (VERBOSE, FREEZE, PARALLEL 0) {TABLE}")
    ...
    try:
        ...
        replica.sql(
            f"ALTER SYSTEM SET primary_conninfo = '{orig_conninfo}'", dbname=TEST_DB
        )
        replica.sql("SELECT pg_reload_conf()", dbname=TEST_DB)
        _wal_receiver_present(replica, True)
