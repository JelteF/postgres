...
def test_checkpoint_logical_slot(create_pg):
    node = create_pg("mike", allows_streaming="logical")
    skip_unless_injection_points(node)
    conn = node.connect()
    conn.sql("CREATE EXTENSION injection_points")

    # Create the two slots we need.
    conn.sql("SELECT pg_create_logical_replication_slot('slot_logical', 'test_decoding')")
    ...
    conn.sql("SELECT count(*) FROM pg_logical_slot_get_changes('slot_logical', null, null)")
    ...
    with node.repeat_query("SELECT 1", interval=0.1):
        ...
        checkpoint_future = checkpoint.asql("CHECKPOINT")
        ...
        # post-recovery check.
        conn.sql("SELECT pg_logical_emit_message(false, '', repeat('123456789', 1000))")

        # Continue the checkpoint and wait for its completion.
        log_offset = node.current_log_position()
        conn.sql("SELECT injection_points_wakeup('checkpoint-before-old-wal-removal')")
    ...
    standby = create_pg(
        ...
        start=False,
        conf=["hot_standby_feedback = on", "primary_slot_name = 'phys_slot'"],
    )
    ...
    standby_checkpoint = standby.background()
    restartpoint_future = standby_checkpoint.asql("CHECKPOINT")
    ...
    standby.sql("SELECT injection_points_wakeup('restartpoint-before-slot-invalidation')")
    standby.sql("SELECT injection_points_detach('restartpoint-before-slot-invalidation')")
    standby_checkpoint.quit()
    ...
    ) is True, "logical slot is not invalidated"
