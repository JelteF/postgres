...
def test_checkpoint_logical_slot(create_pg):
    ...
    with node.repeat_query("SELECT 1", interval=0.1):
        ...
        checkpoint = node.connect()
        checkpoint_future = checkpoint.background_sql("CHECKPOINT")
    ...
    standby_checkpoint = standby.connect()
    restartpoint_future = standby_checkpoint.background_sql("CHECKPOINT")
