...
def test_subtransactions(create_pg):
    ...
    primary.sql("BEGIN; INSERT INTO t_012_tbl VALUES (142); ROLLBACK")
    ...
    primary.sql("BEGIN; SELECT hs_subxids(127); COMMIT")
    ...
    primary.sql("DELETE FROM t_012_tbl")
    primary.sql(HS_SUBXIDS)
    primary.sql("BEGIN; SELECT hs_subxids(127); PREPARE TRANSACTION 'xact_012_1'")
    primary.wait_for_catchup(standby)
    ...
    primary.sql("DELETE FROM t_012_tbl")
    primary.sql("BEGIN; SELECT hs_subxids(201); PREPARE TRANSACTION 'xact_012_1'")
    primary.wait_for_catchup(standby)
    assert standby.sql(SUM_QUERY) == -1, "Not visible"
    primary.stop()
    standby.promote()
