...
def test_effective_wal_level(create_pg):
    ...
    with primary.connect() as conn:
        conn.sql("SELECT pg_create_logical_replication_slot('test_slot2', 'pgoutput')")
        conn.sql("SELECT pg_drop_replication_slot('test_slot2')")
    assert wal_level(primary) == ("replica", "logical"), (
    ...
    with primary.connect() as conn:
        conn.sql("SELECT pg_drop_replication_slot('test_slot')")
        conn.sql("SELECT pg_create_logical_replication_slot('test_slot', 'pgoutput')")
