...
def test_effective_wal_level(create_pg):
    # This test runs many sequential operations across several nodes and many
    # restarts. Give each node a fresh full timeout per poll (as the Perl test
    # does) rather than a single shared per-test deadline.
    ...
    def wait_logical_disabled(node):
        node.poll_query_until("SELECT current_setting('effective_wal_level') = 'replica'")

    primary = create_pg("primary", allows_streaming=True, conf=["log_min_messages = debug1"])
    ...
    primary.sql("SELECT pg_create_physical_replication_slot('test_phy_slot', false, false)")
    assert wal_level(primary) == ("replica", "replica"), (
    ...
    ) is True, "test_slot gets invalidated due to wal_removed"
    ...
    primary.adjust_conf("max_slot_wal_keep_size")
    primary.adjust_conf("min_wal_size")
    primary.adjust_conf("max_wal_size")
    primary.pg_ctl("restart")
    ...
    standby1.sql("SELECT pg_create_logical_replication_slot('standby1_slot2', 'pgoutput')")
    standby1.stop()
    ...
    cascade = create_pg(
        "cascade", from_backup=backup3, streaming_primary=standby2, start=False,
    )
    ...
    assert wal_level(cascade) == ("replica", "logical"), "wal levels on cascaded standby"
    ...
    assert wal_level(primary) == ("replica", "replica"), "decreased to 'replica' on primary"
    assert wal_level(standby2) == ("logical", "replica"), "decreased to 'replica' on standby"
    assert wal_level(cascade) == ("replica", "replica"), "decreased to 'replica' on cascade"
    ...
    assert wal_level(standby3) == ("replica", "replica"), "decreased to 'replica' on standby"
    ...
    creating = primary.background()
    creating.sql("SELECT injection_points_set_local()")
    creating.sql("SELECT injection_points_attach('logical-decoding-activation', 'wait')")
    create_future = creating.asql(
