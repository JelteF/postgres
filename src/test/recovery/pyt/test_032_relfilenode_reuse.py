...
def test_relfilenode_reuse(create_pg, pg_bin):
    primary = create_pg(
        "primary",
        ...
        conf=[
            ...
            "full_page_writes = off",  # to avoid "repairing" corruption
    ...
    psql_primary = primary.background()
    psql_standby = standby.background()
    ...
    assert re.search(
        ...
        pg_bin.run("pg_controldata", str(primary.datadir)).stdout,
    ), "primary shut down ok"
    assert re.search(
        r"Database cluster state:\s+shut down in recovery\n",
        pg_bin.run("pg_controldata", str(standby.datadir)).stdout,
