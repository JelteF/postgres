...
def check_orphan_relfilenodes(conn, datadir, test_name):
    ...
    on_disk = sorted(
        prefix + f
        for f in os.listdir(pathlib.Path(datadir) / prefix)
        if f.isdigit()
    )
...
def test_wal_optimize(create_pg, tmp_path, wal_level):
    ...
    conn.sql("CREATE TABLE moved (id int)")
    conn.sql("INSERT INTO moved VALUES (1)")
    conn.sql(f"CREATE TABLESPACE other LOCATION '{tablespace_dir}'")
    conn.sql_batch(
        "BEGIN;"
    ...
    ) == 2, f"wal_level = {wal_level}, CREATE TABLESPACE, CREATE INDEX"

    # Direct truncation optimization, no tuples.
    conn.sql_batch("BEGIN; CREATE TABLE trunc (id serial PRIMARY KEY); TRUNCATE trunc; COMMIT")
    crash_and_restart()
    ...
    with pytest.raises(LibpqError, match="violates unique"):
        conn.sql("INSERT INTO idx_hint VALUES (2)")
    ...
    check_orphan_relfilenodes(
        conn, node.datadir, f"wal_level = {wal_level}, no orphan relfilenode remains"
    )
