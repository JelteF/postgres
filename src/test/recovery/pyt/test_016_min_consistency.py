...
def test_min_consistency(create_pg, pg_bin):
    ...
    primary = create_pg(
        "primary", allows_streaming=True,
        conf=["shared_buffers = 128kB", "autovacuum = off"],
    )
    ...
    standby_conn = standby.connect()

    primary_conn.sql("CREATE TABLE test1 (a int) WITH (fillfactor = 10)")
    primary_conn.sql("INSERT INTO test1 SELECT generate_series(1, 10000)")
    ...
    control = pg_bin.run("pg_controldata", str(standby.datadir)).stdout
