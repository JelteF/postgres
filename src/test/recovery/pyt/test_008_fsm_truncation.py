...
def test_fsm_truncation(create_pg):
    primary = create_pg(
        ...
        conf=["max_prepared_transactions = 5", "autovacuum = off"],
    )
    ...
    pconn.sql("create table testtab (a int, b char(100))")
    pconn.sql("insert into testtab select generate_series(1,1000), 'foo'")
    pconn.sql("insert into testtab select generate_series(1,1000), 'foo'")
    pconn.sql("delete from testtab where ctid > '(8,0)'")
    ...
    pconn.sql("vacuum verbose testtab")
    ...
    sconn = standby.connect()  # the restart invalidated the held connection

    # INSERT must succeed against the truncated relation's FSM.
    sconn.sql("insert into testtab select generate_series(1,1000), 'foo'")
