...
def test_logical_decoding_timelines(create_pg):
    ...
    with primary.connect() as pconn:
        ...
        pconn.sql("CREATE TABLE decoding(blah text)")
        pconn.sql("INSERT INTO decoding(blah) VALUES ('beforebb')")
    ...
    assert (
        replica.sql("SELECT plugin FROM pg_replication_slots WHERE slot_name = 'dropme_slot'")
        == []
    ...
    with primary.connect() as pconn:
        ...
        pconn.sql("INSERT INTO decoding(blah) VALUES ('afterbb')")
        pconn.sql("CHECKPOINT")
    ...
    replica.promote()
    with replica.connect() as rconn:
        rconn.sql("INSERT INTO decoding(blah) VALUES ('after failover')")
