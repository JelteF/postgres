...
def test_logical_decoding_timelines(create_pg):
    ...
    # Verify that DROP DATABASE on a standby with a logical slot works. The only
    # way to get a logical slot on a standby is the same physical-copy trick.
    ...
    # should give complete results. Find the commit lsn of the last transaction.
    endpos = replica.sql(
