...
def test_overwrite_contrecord(create_pg):
    node = create_pg(
        ...
        conf=["autovacuum = off", "wal_keep_size = 1GB"],
    )
    pconn = node.connect()
    pconn.sql("create table filler (a int, b text)")
    pconn.sql(FILL_WAL)

    initfile = pconn.sql("SELECT pg_walfile_name(pg_current_wal_insert_lsn())")
    pconn.sql("SELECT pg_logical_emit_message(true, 'test 026', repeat('xyzxz', 123456))")
    endfile = pconn.sql("SELECT pg_walfile_name(pg_current_wal_insert_lsn())")
    ...
    pconn.sql("create table foo (a text); insert into foo values ('hello')")
    pconn.sql("SELECT pg_logical_emit_message(true, 'test 026', 'AABBCC')")
