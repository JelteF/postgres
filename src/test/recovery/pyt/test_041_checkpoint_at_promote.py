...
finish the restart point, and the newly-promoted node must remove the WAL
segment and survive a subsequent crash-restart.
"""
...
def test_checkpoint_at_promote(create_pg):
    ...
    primary = create_pg(
        "master", allows_streaming=True,
        conf=["log_checkpoints = on", "restart_after_crash = on"],
    )
    ...
    logstart = standby.current_log_position()
    sconn.sql(f"SELECT injection_points_wakeup('{POINT}')")
