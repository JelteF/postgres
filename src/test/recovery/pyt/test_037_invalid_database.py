...
def test_invalid_database(create_pg):
    ...
    bgpsql = node.background()
    pid = bgpsql.sql("SELECT pg_backend_pid()")
    bgpsql.sql("CREATE DATABASE regression_invalid_interrupt")
    bgpsql.sql("BEGIN; LOCK pg_tablespace; PREPARE TRANSACTION 'lock_tblspc'")

    # Dispatch the DROP; it blocks on the lock.
    drop = bgpsql.asql("DROP DATABASE regression_invalid_interrupt")
    ...
    bgpsql.sql("ROLLBACK PREPARED 'lock_tblspc'")
    bgpsql.sql("DROP DATABASE regression_invalid_interrupt")
    bgpsql.quit()
