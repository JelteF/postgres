...
def test_archiving(create_pg):
    ...
    standby = create_pg(
        ...
        conf=[
            ...
            f"archive_cleanup_command = 'echo done > {cleanup_file}'",
            f"recovery_end_command = 'echo done > {end_file}'",
        ],
    )
    ...
    assert sconn.sql("SELECT count(*) FROM tab_int") == 1000, (
    ...
    standby2.append_conf(filename="recovery.signal")
