...
Two background sessions are used as in the Perl test: ``killme`` holds the
backend that gets signalled, and ``monitor`` runs a long query whose failure
signals that the crash-restart has begun.
"""
...
def test_crash_restart(create_pg):
    ...
    setup = node.connect()
    setup.sql("ALTER SYSTEM SET restart_after_crash = 1")
    setup.sql("ALTER SYSTEM SET log_connections = receipt")
    setup.sql("SELECT pg_reload_conf()")

    setup.sql("CREATE EXTENSION pg_stat_statements")
    stats_reset = setup.sql("SELECT stats_reset FROM pg_stat_statements_info")

    killme = node.background()
    monitor = node.background()
    ...
    killme.sql("BEGIN")
    assert (
        killme.sql("INSERT INTO alive VALUES('in-progress-before-sigquit') "
                   "RETURNING status")
        == "in-progress-before-sigquit"
    )

    # Long-running query in the monitor session; its failure signals the crash.
    monitor_future = monitor.asql("SELECT pg_sleep(3600)")
    ...
    # The crashed sessions are gone; open fresh ones for the SIGKILL round.
    killme = node.background()
    monitor = node.background()
    ...
    assert (
        killme.sql("INSERT INTO alive VALUES('committed-before-sigkill') "
                   "RETURNING status")
        == "committed-before-sigkill"
    )
    killme.sql("BEGIN")
    assert (
        killme.sql("INSERT INTO alive VALUES('in-progress-before-sigkill') "
                   "RETURNING status")
    ...
    assert (
        node.sql("INSERT INTO alive VALUES('before-orderly-restart') "
                 "RETURNING status")
    ...
    assert (
        node.sql("INSERT INTO alive VALUES('after-orderly-restart') "
                 "RETURNING status")
