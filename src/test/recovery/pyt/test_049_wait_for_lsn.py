...
def _resume_walreceiver(node, saved):
    with node.connect() as conn:
        conn.sql(f"ALTER SYSTEM SET primary_conninfo = {saved}")
        conn.sql("SELECT pg_reload_conf()")
...
def _check_fencepost(node, mode, current_lsn, label):
    ...
    with node.connect() as conn:
        lsn_minus = conn.sql(f"SELECT ('{current_lsn}'::pg_lsn - 1)::text")
        lsn_plus = conn.sql(f"SELECT ('{current_lsn}'::pg_lsn + 1)::text")
...
def test_wait_for_lsn(create_pg):
    ...
    standby = create_pg(
        "standby", from_backup=backup, streaming_primary=primary, start=False,
    ...
    def insert_lsn(values):
        primary_conn.sql(f"INSERT INTO wait_test VALUES ({values})")
        return primary_conn.sql("SELECT pg_current_wal_insert_lsn()")
    ...
    assert standby.sql(f"SELECT pg_lsn_cmp(pg_last_wal_replay_lsn(), '{lsn1}'::pg_lsn)") >= 0, (
    ...
    ) >= 0, "standby wrote WAL up to target LSN with MODE 'standby_write'"
    ...
    ) >= 0, "standby flushed WAL up to target LSN with MODE 'standby_flush'"

    lsn_primary_flush = insert_lsn("generate_series(51, 60)")
    primary.sql(f"WAIT FOR LSN '{lsn_primary_flush}' WITH (MODE 'primary_flush', timeout '1d')")
    assert primary.sql(
        f"SELECT pg_lsn_cmp(pg_current_wal_flush_lsn(), '{lsn_primary_flush}'::pg_lsn)"
    ) >= 0, "primary flushed WAL up to target LSN with MODE 'primary_flush'"
    ...
    assert standby.sql(f"WAIT FOR LSN '{lsn2}' WITH (timeout '0.1s', no_throw)") == "success", (
        "WAIT FOR returns correct status after successful waiting"
    )
    assert standby.sql(f"WAIT FOR LSN '{lsn3}' WITH (timeout '10ms', no_throw)") == "timeout", (
    ...
    blocked = sub.background_sql(f"WAIT FOR LSN '{subxact_lsn}' WITH (MODE 'primary_flush')")
    ...
    assert primary.sql(
        ...
        "WHERE application_name = 'wait_for_lsn_subxact_cleanup' "
        "AND wait_event = 'WaitForWalFlush'"
    ) is True, "canceled WAIT FOR LSN in subtransaction"
    ...
    with standby.connect() as c:
        ...
        c.sql("SELECT 1")
        with pytest.raises(
            LibpqError, match="WAIT FOR must be called without an active or registered snapshot"
        ):
    ...
    cases = [
        (f"WAIT FOR LSN '{test_lsn}' WITH (timeout '-1000ms')", "timeout cannot be negative"),
        (f"WAIT FOR LSN '{test_lsn}' WITH (unknown_param 'value')", 'option "unknown_param" not recognized'),
        (f"WAIT FOR LSN '{test_lsn}' WITH (timeout '1000', timeout '2000')", "conflicting or redundant options"),
        (f"WAIT FOR LSN '{test_lsn}' WITH (no_throw, no_throw)", "conflicting or redundant options"),
        ...
        (f"WAIT FOR LSN '{test_lsn}' WITH (timeout 'invalid')", "invalid timeout value"),
        (f"WAIT FOR LSN '{test_lsn}' WITH (invalid_option 'value')", 'option "invalid_option" not recognized'),
        (f"WAIT FOR LSN '{test_lsn}' WITH (MODE 'invalid')", 'unrecognized value for WAIT option "mode": "invalid"'),
        (f"WAIT FOR LSN '{test_lsn}' WITH (MODE 'standby_replay', MODE 'standby_write')", "conflicting or redundant options"),
    ]
    with standby.connect() as conn:
        for query, msg in cases:
            with pytest.raises(LibpqError, match=msg):
                conn.sql(query)

        assert conn.sql(f"WAIT FOR LSN '{lsn2}' WITH (timeout '0.1s', no_throw)") == "success", (
            "WAIT FOR WITH clause syntax works correctly"
        )
        assert conn.sql(f"WAIT FOR LSN '{lsn3}' WITH (timeout 100, no_throw)") == "timeout", (
    ...
    for i in range(5):
        ...
        s = standby.connect()
        replay_waiters.append((s, s.background_sql(f"WAIT FOR LSN '{lsn}'")))
    standby.sql("SELECT pg_wal_replay_resume()")
    for s, fut in replay_waiters:
        fut.result()
        s.close()
    ...
    def multi_mode_waiters(mode, base, wait_event):
        ...
        for i in range(5):
            s = standby.connect()
            waiters.append(
                (s, s.background_sql(f"WAIT FOR LSN '{lsns[i]}' WITH (MODE '{mode}', timeout '1d')"))
            )
    ...
    ) >= 0, "multiple standby_write waiters: standby wrote WAL up to target LSN"
    ...
    ) >= 0, "multiple standby_flush waiters: standby flushed WAL up to target LSN"
    ...
    for i in range(6):
        s = standby.connect()
        mixed.append(
            (s, s.background_sql(f"WAIT FOR LSN '{mixed_target}' WITH (MODE '{modes[i % 3]}', timeout '1d')"))
        )
    standby.poll_query_until(
    ...
    ) is True, "mixed mode waiters: all modes completed and reached target LSN"
    ...
    for i in range(5):
        s = primary.connect()
        pf_waiters.append(
            (s, s.background_sql(f"WAIT FOR LSN '{pf_lsns[i]}' WITH (MODE 'primary_flush', timeout '1d')"))
        )
    for s, fut in pf_waiters:
        fut.result()
        s.close()
    assert primary.sql(
        f"SELECT pg_lsn_cmp(pg_current_wal_flush_lsn(), '{pf_lsns[4]}'::pg_lsn)"
    ) >= 0, "multiple primary_flush waiters: primary flushed WAL up to target LSN"
    ...
    for mode in wait_modes:
        s = standby.connect()
        promote_waiters.append((s, s.background_sql(f"WAIT FOR LSN '{lsn4}' WITH (MODE '{mode}')")))
    ...
    assert standby.sql(f"WAIT FOR LSN '{lsn4}' WITH (timeout '10ms', no_throw)") == "not in recovery", (
    ...
    arc_standby = create_pg("arc_standby", from_backup=arc_backup, restoring=arc_primary)
    ...
    archive_switch(arc_primary)
    arc_w = arc_standby.connect()
    arc_wf = arc_w.background_sql(f"WAIT FOR LSN '{arc_target2}' WITH (MODE 'standby_write', timeout '1d', no_throw)")
    arc_f = arc_standby.connect()
    arc_ff = arc_f.background_sql(f"WAIT FOR LSN '{arc_target2}' WITH (MODE 'standby_flush', timeout '1d', no_throw)")
    arc_standby.poll_query_until(
    ...
    rcv_primary = create_pg("rcv_primary", allows_streaming=True, conf={"autovacuum": False})
    rcv_primary.sql("CREATE TABLE rcv_test AS SELECT generate_series(1,10) AS a")
    rcv_backup = rcv_primary.backup("rcv_backup")
    rcv_standby = create_pg("rcv_standby", from_backup=rcv_backup, streaming_primary=rcv_primary)
    ...
    ) is True, "replay sits ahead of initial walreceiver flush position"
    assert rcv_standby.sql(
        f"SELECT mod(pg_wal_lsn_diff('{rcv_receive}'::pg_lsn, '0/0'::pg_lsn), setting::numeric)::int "
        "FROM pg_settings WHERE name = 'wal_segment_size'"
    ) == 0, "initial walreceiver flush position is segment-aligned"

    with rcv_standby.connect() as conn:
        for mode in ("standby_write", "standby_flush"):
            assert conn.sql(
                f"WAIT FOR LSN '{rcv_replay}' WITH (MODE '{mode}', timeout '5s', no_throw)"
            ) == "success", f"{mode} succeeds for already-replayed LSN after standby restart"
    ...
    _, replay_lsn_plus = _check_fencepost(rcv_standby, "standby_replay", replay_lsn, "standby_replay")

    flush_lsn = rcv_standby.sql("SELECT pg_last_wal_receive_lsn()")
    assert rcv_standby.sql(
        f"SELECT pg_wal_lsn_diff('{flush_lsn}'::pg_lsn, '{replay_lsn}'::pg_lsn) >= 0"
    ) is True, "standby_flush boundary is not masked by replay floor"
    ...
    boundary = rcv_standby.connect()
    boundary_fut = boundary.background_sql(
    ...
    tl_primary = create_pg("tl_primary", allows_streaming=True, conf={"autovacuum": False})
    tl_primary.sql("CREATE TABLE tl_test AS SELECT generate_series(1, 10) AS a")
    tl_backup = tl_primary.backup("tl_backup")
    tl_standby1 = create_pg("tl_standby1", from_backup=tl_backup, streaming_primary=tl_primary)
    tl_backup2 = tl_standby1.backup("tl_backup2")
    tl_standby2 = create_pg("tl_standby2", from_backup=tl_backup2, streaming_primary=tl_standby1)
    ...
    tl_session = tl_standby2.connect()
    tl_fut = tl_session.background_sql(
