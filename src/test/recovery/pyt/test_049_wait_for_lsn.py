...
here a blocked waiter is dispatched with ``asql()`` and its completion observed
by resolving the returned future, so the log helpers are unnecessary.
"""
...
def test_wait_for_lsn(create_pg):
    # Long multi-section test: give each node a fresh full timeout per poll
    # rather than a single shared per-test deadline.
    ...
    blocked = sub.asql(f"WAIT FOR LSN '{subxact_lsn}' WITH (MODE 'primary_flush')")
    ...
    assert standby.sql(f"WAIT FOR LSN '{lsn3}' WITH (timeout 100, no_throw)") == "timeout", (
    ...
    for i in range(5):
        ...
        s = standby.background()
        replay_waiters.append((s, s.asql(f"WAIT FOR LSN '{lsn}'")))
    ...
    def multi_mode_waiters(mode, base, wait_event):
        ...
        for i in range(5):
            s = standby.background()
            waiters.append(
                (s, s.asql(f"WAIT FOR LSN '{lsns[i]}' WITH (MODE '{mode}', timeout '1d')"))
            )
    ...
    for i in range(6):
        s = standby.background()
        mixed.append(
            (s, s.asql(f"WAIT FOR LSN '{mixed_target}' WITH (MODE '{modes[i % 3]}', timeout '1d')"))
        )
    standby.poll_query_until(
    ...
    for i in range(5):
        ...
        pf_waiters.append(
            (s, s.asql(f"WAIT FOR LSN '{pf_lsns[i]}' WITH (MODE 'primary_flush', timeout '1d')"))
        )
    ...
    for mode in wait_modes:
        s = standby.background()
        promote_waiters.append((s, s.asql(f"WAIT FOR LSN '{lsn4}' WITH (MODE '{mode}')")))
    ...
    archive_switch(arc_primary)
    arc_w = arc_standby.background()
    arc_wf = arc_w.asql(f"WAIT FOR LSN '{arc_target2}' WITH (MODE 'standby_write', timeout '1d', no_throw)")
    arc_f = arc_standby.background()
    arc_ff = arc_f.asql(f"WAIT FOR LSN '{arc_target2}' WITH (MODE 'standby_flush', timeout '1d', no_throw)")
    arc_standby.poll_query_until(
    ...
    rcv_primary = create_pg("rcv_primary", allows_streaming=True, conf=["autovacuum = off"])
    ...
    boundary = rcv_standby.background()
    boundary_fut = boundary.asql(
    ...
    tl_primary = create_pg("tl_primary", allows_streaming=True, conf=["autovacuum = off"])
    ...
    tl_session = tl_standby2.background()
    tl_fut = tl_session.asql(
