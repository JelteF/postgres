...
def test_invalidate_inactive_slots(create_pg):
    node = create_pg(
        ...
        conf=["checkpoint_timeout = 1h", "idle_replication_slot_timeout = 1min"],
    )
    ...
    conn.sql("SELECT pg_create_logical_replication_slot('logical_slot', 'test_decoding')")
    ...
    conn.sql("SELECT injection_points_attach('slot-timeout-inval', 'error')")
    conn.sql("CHECKPOINT")
    ...
    with pytest.raises(
        ...
        conn.sql("SELECT pg_replication_slot_advance('logical_slot', '0/1')")
