...
def test_replslot_limit(create_pg):
    ...
    primary = create_pg(
        ...
        conf=["min_wal_size = 2MB", "max_wal_size = 4MB", "log_checkpoints = yes"],
    )
    ...
    standby = create_pg(
        "standby_1", from_backup=backup, streaming_primary=primary, start=False,
    ...
    primary.sql("CHECKPOINT")
    assert wal_status() == "reserved", "check that slot remains reserved after advancing WAL"
    ...
    assert not re.search(
        r"requested WAL segment [0-9A-F]+ has already been removed", standby.log_since(0)
    ...
    primary2 = create_pg(
        "primary2",
        allows_streaming=True,
        conf=["min_wal_size = 32MB", "max_wal_size = 32MB", "log_checkpoints = yes"],
    )
    ...
    standby2 = create_pg(
        "standby_2", from_backup=backup2, streaming_primary=primary2, start=False,
        conf=["primary_slot_name = 'rep1'"],
    )
    standby2.start()
    primary2.advance_wal(1)
    assert primary2.sql("CHECKPOINT; SELECT 'finished'") == "finished", (
    ...
    standby3 = create_pg(
        "standby_3", from_backup=backup3, streaming_primary=primary3, start=False,
    ...
    primary4 = create_pg("primary4", allows_streaming=True, conf=["wal_level = logical"])
    backup4 = primary4.backup("my_backup4")
    standby4 = create_pg(
        "standby4", from_backup=backup4, streaming_primary=primary4, start=False,
    ...
    ) is True, "last inactive time for an active physical slot is NULL"
    ...
    ) is True, "last inactive time for an inactive physical slot is updated correctly"
    ...
    assert publisher4.sql(
        "SELECT inactive_since IS NULL FROM pg_replication_slots WHERE slot_name = 'lsub4_slot'"
    ) is True, "last inactive time for an active logical slot is NULL"
    ...
    ) is True, "last inactive time for an inactive logical slot is updated correctly"
