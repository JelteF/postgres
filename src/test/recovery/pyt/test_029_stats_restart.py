...
def test_stats_restart(create_pg, tmp_path):
    ...
    # Held connection for the stats queries (on 'postgres'), refreshed after
    # each (re)start since those invalidate it. The 'test' database is only
    # touched by the workload below; connecting to it eagerly would create
    # database stats and break the "stats were discarded" checks.
    cconn = node.connect(dbname=connect_db)
    ...
    def have_stats(kind, dboid, objid):
        return cconn.sql(f"SELECT pg_stat_have_stats('{kind}', {dboid}, {objid})")
    ...
    def checkpoint_stats():
        ...
        reset = cconn.sql("SELECT stats_reset FROM pg_stat_checkpointer")
        return {"count": count, "reset": reset}

    def wal_stats():
        records = cconn.sql("SELECT wal_records FROM pg_stat_wal")
        wal_bytes = cconn.sql("SELECT wal_bytes FROM pg_stat_wal")
        reset = cconn.sql("SELECT stats_reset FROM pg_stat_wal")
    ...
    def trigger_funcrel_stat():
        ...
        node.connect(dbname=db_under_test).sql(
    ...
    cconn.sql(f"CREATE DATABASE {db_under_test}")
    ...
    assert ckpt_start["count"] < ckpt_restart["count"], "post restart: increased checkpoint count"
    assert wal_start["records"] < wal_restart["records"], "post restart: increased wal record count"
    assert wal_start["bytes"] < wal_restart["bytes"], "post restart: increased wal bytes"
    assert ckpt_start["reset"] == ckpt_restart["reset"], "post restart: checkpoint stats_reset equal"
    ...
    cconn.sql("SELECT pg_stat_reset_shared('checkpointer')")
    ckpt_reset = checkpoint_stats()
    wal_ckpt_reset = wal_stats()
    assert ckpt_restart["count"] > ckpt_reset["count"], "post ckpt reset: checkpoint count smaller"
    ...
    assert wal_start["reset"] == wal_ckpt_reset["reset"], "post ckpt reset: wal stats_reset equal"
    ...
    cconn.sql("SELECT pg_stat_reset_shared('wal')")
    wal_reset = wal_stats()
    assert wal_reset["records"] < wal_restart2["records"], "post wal reset: smaller record count"
    ...
    assert wal_reset["bytes"] < wal_restart2["bytes"], "post wal reset & restart: smaller bytes"
    assert wal_reset["reset"] > wal_restart2["reset"], "post wal reset & restart: newer stats_reset"
