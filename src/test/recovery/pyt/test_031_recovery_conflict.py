...
def test_recovery_conflict(create_pg):
    primary = create_pg(
        ...
        conf=[
            ...
            f"temp_tablespaces = {TABLESPACE1}",
    ...
    bg = standby.background(dbname=TEST_DB)
    res = bg.sql(
        f"BEGIN; DECLARE {CURSOR1} CURSOR FOR SELECT b FROM {TABLE1}; "
    ...
    expected_conflicts += 1
    bg = standby.background(dbname=TEST_DB)
    res = bg.sql(f"BEGIN; LOCK TABLE {TABLE1} IN ACCESS SHARE MODE; SELECT 1")
    ...
    standby.append_conf(f"max_standby_streaming_delay = '{test_timeout_default()}s'")
    ...
    primary.sql(f"CREATE TABLE {TABLE1}(a int, b int)", dbname=TEST_DB)
    primary.sql(f"INSERT INTO {TABLE1} VALUES (1)", dbname=TEST_DB)
    with primary.connect(dbname=TEST_DB) as c:
        c.sql("BEGIN")
        ...
        c.sql(f"BEGIN; LOCK TABLE {TABLE2}; PREPARE TRANSACTION 'lock'")
    primary.sql(f"INSERT INTO {TABLE1}(a) VALUES (170)", dbname=TEST_DB)
    primary.sql("SELECT txid_current()", dbname=TEST_DB)
    primary.wait_for_catchup(standby)
    ...
    waiter = bg.asql(f"SELECT * FROM {TABLE2}")
    ...
    db_conn = standby.background(dbname=TEST_DB)
    db_conn.sql("SELECT 1")
    offset = standby.current_log_position()
    primary.sql(f"DROP DATABASE {TEST_DB}")
    primary.wait_for_catchup(standby)
    standby.wait_for_log("User was connected to a database that must be dropped", offset)
    db_conn.quit()
