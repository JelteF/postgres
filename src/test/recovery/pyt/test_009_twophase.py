...
def test_twophase(create_pg, pg_bin, tmp_path):
    london = create_pg(
        "london",
    ...
    def configure_and_reload(node, parameter):
        node.append_conf(parameter)
        assert node.sql("SELECT pg_reload_conf()") is True, (
            f"reload node {node.name} with {parameter}"
        )
    ...
    standby_session = standby.background()
    ...
    pconn.sql("BEGIN; CREATE TABLE test1(); PREPARE TRANSACTION 'foo'")
