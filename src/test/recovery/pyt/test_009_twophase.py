...
EXPECTED_TBL = [
    (1, "issued to london"), (2, "issued to london"),
    (5, "issued to london"), (6, "issued to london"),
    (9, "issued to london"), (10, "issued to london"),
    (11, "issued to london"), (12, "issued to london"),
    (13, "issued to london"), (14, "issued to london"),
    (15, "issued to london"), (16, "issued to london"),
    (17, "issued to london"), (18, "issued to london"),
    (19, "issued to london"), (20, "issued to london"),
    (21, "issued to london"), (22, "issued to london"),
    (23, "issued to paris"), (24, "issued to paris"),
    (25, "issued to london"), (26, "issued to london"),
]
...
def test_twophase(create_pg, tmp_path):
    ...
    with primary.connect() as c:
        c.sql("COMMIT PREPARED 'xact_009_1'")
        c.sql("ROLLBACK PREPARED 'xact_009_2'")
    ...
    primary.start()
    with primary.connect() as c:
        c.sql("COMMIT PREPARED 'xact_009_3'")
        c.sql("ROLLBACK PREPARED 'xact_009_4'")
    ...
    with primary.connect() as c:
        ...
        c.sql_batch(
            ...
            f" PREPARE TRANSACTION 'xact_009_standby_mvcc'"
        )
    primary.stop()
    standby.pg_ctl("restart")
    ...
    primary.sql_batch(
        f"BEGIN;"
        f" CREATE TABLE t_009_tbl2 (id int, msg text);"
        f" SAVEPOINT s1;"
        f" INSERT INTO t_009_tbl2 VALUES (27, 'issued to {name}');"
        f" PREPARE TRANSACTION 'xact_009_13'"
    )
    ...
    primary.start()
    with primary.connect() as c:
        c.sql("COMMIT PREPARED 'xact_009_14'")
        c.sql("ROLLBACK PREPARED 'xact_009_15'")
    ...
    primary.start()
    with primary.connect() as c:
        c.sql("COMMIT PREPARED 'xact_009_16'")
        c.sql("ROLLBACK PREPARED 'xact_009_17'")

    # Expected data on both servers. The roles are stable from here, so hold a
    # connection to each rather than reconnecting per query.
    pconn = primary.connect()
    sconn = standby.connect()
    assert pconn.sql("SELECT count(*) FROM pg_prepared_xacts") == 0, (
        "No uncommitted prepared transactions on primary"
    )
    assert pconn.sql("SELECT * FROM t_009_tbl ORDER BY id") == EXPECTED_TBL, (
        "Check expected t_009_tbl data on primary"
    )
    assert pconn.sql("SELECT * FROM t_009_tbl2") == (27, "issued to paris"), (
        "Check expected t_009_tbl2 data on primary"
    )
    assert sconn.sql("SELECT count(*) FROM pg_prepared_xacts") == 0, (
        "No uncommitted prepared transactions on standby"
    )
    assert sconn.sql("SELECT * FROM t_009_tbl ORDER BY id") == EXPECTED_TBL, (
        "Check expected t_009_tbl data on standby"
    )
    assert sconn.sql("SELECT * FROM t_009_tbl2") == (27, "issued to paris"), (
    ...
    pconn.sql("CHECKPOINT")
    pconn.sql("CREATE TABLE test()")
    pconn.sql_batch("BEGIN; CREATE TABLE test1(); PREPARE TRANSACTION 'foo'")

    subtrans_query = (
        "select 'pg_subtrans/'||f, s.size "
        "from pg_ls_dir('pg_subtrans') f, pg_stat_file('pg_subtrans/'||f) s"
    )
    osubtrans = pconn.sql(subtrans_query)
