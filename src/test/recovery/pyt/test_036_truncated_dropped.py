...
def test_truncated_dropped(create_pg):
    node = create_pg("n1", conf=["wal_level = 'replica'", "autovacuum = off"])
    conn = node.connect()
    ...
    def crash_and_restart():
        ...
        node.stop("immediate")
        node.start()
        conn = node.connect()  # the restart invalidated the held connection
    ...
    crash_and_restart()
    assert conn.sql("select count(*), sum(i) FROM truncme") == (10, 55), (
        "table contents as expected after recovery"
    )
    conn.sql("DROP TABLE truncme")
