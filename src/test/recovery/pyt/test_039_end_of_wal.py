...
def test_end_of_wal(create_pg):
    ...
    # xl_tot_len is > XLogRecordMaxSize (also recycled garbage).
    node.emit_wal(0)
    end_lsn = node.advance_wal_out_of_record_splitting_zone(wal_block_size)
    expect_recovery_log(
        "invalid record length at .*: expected at most 1069547520, got 1073741824",
        (end_lsn, _build_record_header(1024 * 1024 * 1024)),
    )
    ...
    expect_recovery_log(
        ...
        (end_lsn, _build_record_header(2 * 1024 * 1024, 0, 0xDEADBEEF)),
    )
    ...
    expect_recovery_log(
        ...
        (end_lsn, _build_record_header(2 * 1024 * 1024, 0, prev_lsn)),
    )
    ...
    expect_recovery_log(
        ...
        (end_lsn, _build_record_header(2 * 1024 * 1024, 42, prev_lsn)),
        (
