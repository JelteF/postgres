...
def _scan_server_header(pg_bin, header_path, pattern):
    """Return the first capture of ``pattern`` in an installed server header,
    mirroring Perl's scan_server_header()."""
    includedir = pg_bin.run("pg_config", "--includedir-server", check=True).stdout.strip()
...
def _build_record_header(xl_tot_len, xl_xid=0, xl_prev=0, xl_info=0, xl_rmid=0, xl_crc=0):
...
def _build_page_header(xlp_magic, xlp_info=0, xlp_tli=0, xlp_pageaddr=0, xlp_rem_len=0):
    ...
    return struct.pack("=HHIQI", xlp_magic, xlp_info, xlp_tli, xlp_pageaddr, xlp_rem_len)


def test_end_of_wal(create_pg, pg_bin):
    page_magic = int(
        _scan_server_header(
            pg_bin, "access/xlog_internal.h", r"#define\s+XLOG_PAGE_MAGIC\s+(\w+)"
        ),
        16,
    )
    first_is_contrecord = int(
        _scan_server_header(
            pg_bin, "access/xlog_internal.h", r"#define\s+XLP_FIRST_IS_CONTRECORD\s+(\w+)"
        ),
        16,
    )
    ...
    conn = node.connect()
    conn.sql("CREATE TABLE t AS SELECT 42")
    ...
    tli = conn.sql("SELECT timeline_id FROM pg_control_checkpoint()")
    ...
    def expect_recovery_log(pattern, *writes):
        ...
        # Reconnect: the immediate stop invalidated the held connection.
        return node.connect()
