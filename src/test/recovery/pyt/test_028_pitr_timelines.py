...
def test_pitr_timelines(create_pg):
    ...
    pconn.sql("INSERT INTO foo VALUES(1)")
    ...
    standby = create_pg(
        "standby", from_backup=backup, streaming_primary=primary,
        conf={"archive_mode": "always"},
    )
    primary.wait_for_catchup(standby)
    sconn = standby.connect()
    assert sconn.sql("SELECT max(i) FROM foo") == 2, (
    ...
    node_pitr = create_pg(
        "node_pitr", from_backup=backup, restoring=primary, restoring_standby=False,
    ...
    assert pitr_conn.sql("SELECT max(i) FROM foo") == 1, (
    ...
    node_pitr2 = create_pg(
        "node_pitr2", from_backup=backup, restoring=primary, restoring_standby=False,
