...
def _test_tablespace(create_pg, strategy):
    ...
    pconn.sql("SET allow_in_place_tablespaces = on")
    for ts in ("dropme_ts1", "dropme_ts2", "source_ts", "target_ts"):
        pconn.sql(f"CREATE TABLESPACE {ts} LOCATION ''")
    pconn.sql("CREATE DATABASE template_db IS_TEMPLATE = true")
    pconn.sql("SELECT pg_create_physical_replication_slot('slot', true)")
    ...
    standby = create_pg(
        ...
        conf=["allow_in_place_tablespaces = on", "primary_slot_name = slot"],
    )
...
def _test_missing_tablespace_dir(create_pg):
    ...
    pconn = primary.connect()
    pconn.sql("SET allow_in_place_tablespaces = on")
    pconn.sql("CREATE TABLESPACE ts1 LOCATION ''")
    pconn.sql("CREATE DATABASE db1 WITH TABLESPACE ts1 STRATEGY=FILE_COPY")
    ...
    standby = create_pg(
        ...
        from_backup=backup,
        streaming_primary=primary,
        conf=["allow_in_place_tablespaces = on"],
    )
    # Make sure the standby reached consistency and accepts connections.
    standby.poll_query_until("SELECT 1", 1)
    ...
    pconn.sql("CREATE TABLE should_not_replay_insertion(a int)")
    pconn.sql("CREATE DATABASE db2 WITH TABLESPACE ts1 STRATEGY=FILE_COPY")
    pconn.sql("INSERT INTO should_not_replay_insertion VALUES (1)")
