...
def test_stats_cleanup_replica(create_pg):
    ...
    sconn = standby.connect()  # standby/postgres; reconnected after restarts

    def have_stats(kind, dboid, objid):
        return sconn.sql(f"SELECT pg_stat_have_stats('{kind}', {dboid}, {objid})")
    ...
    dboid, tableoid, funcoid = populate(pconn, "public", "postgres")
    check_func_tab(dboid, tableoid, funcoid, True)

    pconn.sql(f"DROP TABLE {pconn.sql(f'SELECT {tableoid}::regclass')}")
    pconn.sql(f"DROP FUNCTION {pconn.sql(f'SELECT {funcoid}::regprocedure')}")
    primary.wait_for_catchup(standby)
    check_func_tab(dboid, tableoid, funcoid, False)

    # Cleaned up after an indirect drop via DROP SCHEMA CASCADE.
    pconn.sql("CREATE SCHEMA drop_schema_test1")
    primary.wait_for_catchup(standby)
    dboid, tableoid, funcoid = populate(pconn, "drop_schema_test1", "postgres")
    check_func_tab(dboid, tableoid, funcoid, True)
    pconn.sql("DROP SCHEMA drop_schema_test1 CASCADE")
    primary.wait_for_catchup(standby)
    check_func_tab(dboid, tableoid, funcoid, False)

    # Cleaned up after dropping the whole database.
    pconn.sql("CREATE DATABASE test")
    primary.wait_for_catchup(standby)
