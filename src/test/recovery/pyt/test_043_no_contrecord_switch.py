...
def test_no_contrecord_switch(create_pg):
    ...
    pconn.sql("CREATE TABLE t AS SELECT 0")

    wal_segment_size = _int_setting(pconn, "wal_segment_size")
    wal_block_size = _int_setting(pconn, "wal_block_size")
    tli = pconn.sql("SELECT timeline_id FROM pg_control_checkpoint()")
    ...
    with standby1.connect() as c:
        c.sql("SELECT pg_switch_wal()")
        c.sql("INSERT INTO t SELECT * FROM generate_series(1, 1000)")
