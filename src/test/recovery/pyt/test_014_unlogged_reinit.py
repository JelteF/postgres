...
def test_unlogged_reinit(create_pg, tmp_path):
    ...
    conn.sql("CREATE UNLOGGED TABLE base_unlogged (id int)")
    conn.sql("CREATE UNLOGGED SEQUENCE seq_unlogged")

    base_table = conn.sql("SELECT pg_relation_filepath('base_unlogged')")
    base_seq = conn.sql("SELECT pg_relation_filepath('seq_unlogged')")
    ...
    ts1_table = conn.sql("SELECT pg_relation_filepath('ts1_unlogged')")
    ...
    conn.sql("CREATE UNLOGGED SEQUENCE seq_unlogged2")
    conn.sql("ALTER SEQUENCE seq_unlogged2 INCREMENT 2")  # rewrites in AlterSequence()
    conn.sql("SELECT nextval('seq_unlogged2')")

    conn.sql(
        "CREATE UNLOGGED TABLE tab_seq_unlogged3 "
        "(a int GENERATED ALWAYS AS IDENTITY)"
    )
    conn.sql("TRUNCATE tab_seq_unlogged3 RESTART IDENTITY")  # rewrites in ResetSequence()
    conn.sql("INSERT INTO tab_seq_unlogged3 DEFAULT VALUES")
    ...
    node.start()
    conn = node.connect()  # the crash invalidated the held connection

    assert (pgdata / f"{base_table}_init").is_file(), (
        "table init fork in base still exists"
    )
    assert (pgdata / base_table).is_file(), "table main fork in base recreated at startup"
    assert not (pgdata / f"{base_table}_vm").exists(), "vm fork in base removed at startup"
    ...
    assert conn.sql("SELECT nextval('seq_unlogged')") == 1, "sequence nextval after restart"
    assert conn.sql("SELECT nextval('seq_unlogged')") == 2, (
        "sequence nextval after restart again"
    )

    assert (pgdata / f"{ts1_table}_init").is_file(), "init fork still exists in tablespace"
    assert (pgdata / ts1_table).is_file(), "main fork in tablespace recreated at startup"
    ...
    assert conn.sql("SELECT nextval('seq_unlogged2')") == 1, (
        "altered sequence nextval after restart"
    )
    assert conn.sql("SELECT nextval('seq_unlogged2')") == 3, (
        "altered sequence nextval after restart again"
    )

    conn.sql("INSERT INTO tab_seq_unlogged3 VALUES (DEFAULT), (DEFAULT)")
    assert conn.sql("SELECT * FROM tab_seq_unlogged3") == [1, 2], (
