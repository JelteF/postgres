...
def test_save_logical_slots_shutdown(create_pg, pg_bin):
    publisher = create_pg(
        # Avoid a checkpoint during the test, which would move the latest
        # checkpoint location.
        "pub",
        allows_streaming="logical",
        conf=["checkpoint_timeout = 1h", "autovacuum = off"],
    )
    subscriber = create_pg("sub")

    pconn = publisher.connect()
    sconn = subscriber.connect()
    pconn.sql("CREATE TABLE test_tbl (id int)")
    sconn.sql("CREATE TABLE test_tbl (id int)")
    ...
    pconn.sql("INSERT INTO test_tbl VALUES (generate_series(1, 5))")

    pconn.sql("CREATE PUBLICATION pub FOR ALL TABLES")
    sconn.sql(
    ...
    assert sconn.sql("SELECT count(*) FROM test_tbl") == 5, "check initial copy was done"
    ...
    control = pg_bin.run("pg_controldata", str(publisher.datadir)).stdout
