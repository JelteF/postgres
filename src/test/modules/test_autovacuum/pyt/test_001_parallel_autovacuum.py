...
def test_parallel_autovacuum(create_pg):
    ...
    # so they can share a single connection.
    with node.connect() as conn:
        conn.sql("ALTER SYSTEM SET autovacuum_vacuum_cost_limit = 500")
        conn.sql("ALTER SYSTEM SET autovacuum_vacuum_cost_delay = 5")
        conn.sql("ALTER SYSTEM SET vacuum_cost_page_miss = 10")
        conn.sql("ALTER SYSTEM SET vacuum_cost_page_dirty = 10")
        conn.sql("ALTER SYSTEM SET vacuum_cost_page_hit = 10")
