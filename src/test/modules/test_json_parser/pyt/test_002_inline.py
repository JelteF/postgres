...
CASES = [
    ...
    ("interrupted escapes", b'"' + b"\\" * 3 + b'"' + b"\\" * 5 + b'"' + b"\\" * 2 + b'"', None),
    ("whitespace", b'     ""     ', None),
    ...
    ("second numeric key", b'{"a": "a", 1234: 2}', r'Expected string, but found "1234"'),
    ("unclosed object with pair", b'{"key": "value"', r"input string ended unexpectedly"),
    ("missing key value", b'{"key": }', r'Expected JSON value, but found "}"'),
    ("missing colon", b'{"key" 12345}', r'Expected ":", but found "12345"'),
    ("missing comma", b'{"key": 12345 12345}', r'Expected "," or "}", but found "12345"'),
    ("overnested array", b"[" * 6401, r"maximum permitted depth is 6400"),
    ("overclosed array", b"[]]", r'Expected end of input, but found "]"'),
    ("unexpected token in array", b"[ }}} ]", r'Expected array element or "]", but found "}"'),
    ...
    ("unknown escape sequence", b'"hello\\vworld"', r'Escape sequence "\\v" is invalid'),
    ("unescaped control", b'"hello\tworld"', r"Character with value 0x09 must be escaped"),
    ("incorrect escape count", b'"' + b"\\" * 7 + b'"', r'Token ""' + r"\\" * 7 + r'"" is invalid'),
    ...
    ("incomplete UTF-8 sequence", b'"\\\xf5', r'(Token|Escape sequence) ""?\\\xf5" is invalid'),
]
...
def test_inline(exe, tmp_path):
    ...
    def check(name, data, error):
        ...
        r = run(*exe, "-r", chunk, fname, check=False,
