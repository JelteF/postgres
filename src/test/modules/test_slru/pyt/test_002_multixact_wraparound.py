...
def test_multixact_wraparound(create_pg, pg_bin):
    ...
    out = pg_bin.run("pg_resetwal", "--dry-run", str(pgdata), check=True).stdout
    blcksz = int(re.search(r"^Database block size: *(\d+)$", out, re.M).group(1))
    pages_per_segment = int(
        re.search(r"^Pages per SLRU segment: *(\d+)$", out, re.M).group(1)
    )
    ...
    multixact_ids = [
        int(node.sql("SELECT test_create_multixact()")) for _ in range(16)
    ]
