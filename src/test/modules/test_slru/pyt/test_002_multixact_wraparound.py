...
def test_multixact_wraparound(create_pg):
    ...
    node.start()
    # Everything below runs on the freshly started server with no further
    # bounce, so share a single connection.
    conn = node.connect()
    conn.sql("CREATE EXTENSION test_slru")
    multixact_ids = [
        int(conn.sql("SELECT test_create_multixact()")) for _ in range(16)
    ]
    ...
    for i, multi in enumerate(multixact_ids):
        assert conn.sql(f"SELECT test_read_multixact('{multi}')") == "", (
