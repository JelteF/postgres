...
def aio_node(request, create_pg_module, pg_bin):
    ...
    conf = [*CONFIGURE, f"io_method = {method}"]
    if method == "sync":
        conf.append("io_max_concurrency = 4")
...
def test_handle(aio_node):
    ...
    with leaks():
        ...
        with pytest.raises(LibpqError, match="release in unexpected state"):
            conn.sql("SELECT handle_release_last()")
        conn.sql("COMMIT")

    # No leak: handle released in the same command, so nothing is warned.
    with no_messages():
        conn.sql("BEGIN")
        conn.sql("SELECT handle_get() UNION ALL SELECT handle_release_last()")
        conn.sql("COMMIT")
        conn.sql("SELECT handle_get_release()")

    # API violation: two handouts.
    with pytest.raises(LibpqError, match="API violation: Only one IO can be handed out"):
        conn.sql("SELECT handle_get_twice()")
    ...
    with pytest.raises(LibpqError, match="as you command"):
        conn.sql("SELECT handle_get_and_error()")
    conn.sql("SELECT 'ok', handle_get_release()")

    conn.sql("BEGIN")
    with pytest.raises(LibpqError, match="as you command"):
        conn.sql("SELECT handle_get_and_error()")
    conn.sql("ROLLBACK")
    conn.sql("SELECT handle_get_release(), 'ok'")

    conn.sql("BEGIN")
    conn.sql("SAVEPOINT foo")
    with pytest.raises(LibpqError, match="as you command"):
        conn.sql("SELECT handle_get_and_error()")
    conn.sql("ROLLBACK TO SAVEPOINT foo")
    conn.sql("SELECT handle_get_release()")
    conn.sql("ROLLBACK")
...
def test_batchmode(aio_node):
    ...
    with no_messages():
        conn.sql(f"{batch_start} UNION ALL SELECT WHERE batch_end() IS NULL")
...
def test_inject(aio_node):
    ...
    with pytest.raises(LibpqError, match=r'invalid page in block 1 of relation "base/.*'):
...
def test_read_buffers(aio_node):
    ...
    for table in ("tbl_ok", "tmp_ok"):
        ...
        assert rb(table, 1, 3, cols_nf) == [(0, 1, True, 1), (1, 2, False, 1), (2, 3, True, 1)]
        a.sql(f"SELECT evict_rel('{table}')")
...
def test_zero(aio_node):
    ...
    for persistency in ("normal", "temporary"):
        kind = "" if persistency == "normal" else persistency
        a.sql(
            f"CREATE {kind} TABLE tbl_zero(id int) WITH (AUTOVACUUM_ENABLED = false)"
        )
        ...
        with pytest.raises(
            ...
            a.sql("SELECT read_rel_block_ll('tbl_zero', 1, nblocks=>4, zero_on_error=>false)")
        # zeroed via the ZERO_ON_ERROR flag ...
        with pytest.warns(
            PostgresWarning,
            match=r"zeroing out 2 invalid pages among blocks 1\.\.4 of relation",
        ):
            a.sql("SELECT read_rel_block_ll('tbl_zero', 1, nblocks=>4, zero_on_error=>true)")
...
def test_checksum(aio_node):
    ...
    with pytest.raises(
        ...
        a.sql("SELECT read_rel_block_ll('tbl_normal', 3, nblocks=>1, zero_on_error=>false)")
    ...
    with pytest.raises(
        ...
        a.sql("SELECT read_rel_block_ll('tbl_temp', 4, nblocks=>2, zero_on_error=>false)")
    _assert_checksum_increased(node, before, "postgres")

    # shared rel
    before = _checksum_count(a, None)
    with pytest.raises(
        LibpqError, match=r'2 invalid pages among blocks 2\.\.3 of relation "global/\d+"'
    ):
        a.sql("SELECT read_rel_block_ll('pg_shseclabel', 2, nblocks=>2, zero_on_error=>false)")
...
def test_ignore_checksum(aio_node):
    ...
    invalidate = (
        "SELECT invalidate_rel_block('tbl_cs_fail', g.i) FROM generate_series(0, 6) g(i)"
    )
    ...
    with pytest.warns(PostgresWarning, match=r"ignoring checksum failure in block 3"):
        conn.sql("SELECT read_rel_block_ll('tbl_cs_fail', 3, nblocks=>1, zero_on_error=>false)")
    node.wait_for_log(r"LOG:  ignoring checksum failure", offset)
    with pytest.raises(
        LibpqError, match=r'invalid page in block 4 of relation "base/\d+/\d+"'
    ):
        conn.sql("SELECT read_rel_block_ll('tbl_cs_fail', 2, nblocks=>3, zero_on_error=>false)")
    ...
    conn.sql("SELECT modify_rel_block('tbl_cs_fail', 3, corrupt_checksum=>true, corrupt_header=>true)")
    conn.sql("SELECT modify_rel_block('tbl_cs_fail', 4, corrupt_header=>true)")
    ...
    with pytest.warns(
        ...
        conn.sql("SELECT read_rel_block_ll('tbl_cs_fail', 1, nblocks=>5, zero_on_error=>true)")
    ...
    with pytest.warns(
        ...
        conn.sql("SELECT read_rel_block_ll('tbl_cs_fail', 3, nblocks=>1, zero_on_error=>true)")
...
def test_checksum_createdb(aio_node):
    ...
    src.sql("CREATE TABLE tbl_cs_fail(data int not null) WITH (AUTOVACUUM_ENABLED = false)")
