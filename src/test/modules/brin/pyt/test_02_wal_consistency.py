...
def test_wal_consistency(create_pg):
    primary = create_pg("brin_whiskey", allows_streaming=True)
    primary.append_conf("wal_consistency_checking = brin")
    ...
    standby = create_pg("brin_charlie", from_backup=backup, streaming_primary=primary)
