...
def test_workitems(create_pg):
    ...
    # Setup and the reads below all run on one server with no bounce, so share
    # a single connection.
    conn = node.connect()

    conn.sql("create extension pageinspect")

    # A table with an autosummarizing BRIN index, ...
    conn.sql("create table brin_wi (a int) with (fillfactor = 10)")
    conn.sql(
    ...
    conn.sql("create table journal (d timestamp) with (fillfactor = 10)")
    conn.sql(
    ...
    conn.sql("insert into brin_wi select * from generate_series(1, 100)")
    conn.sql(
