...
is psql's ``\\bind ... \\g``, prepare()/exec_prepared() are ``\\parse`` /
``\\bind_named``, and the pipeline() context manager is
...
def test_log_temp_files(create_pg):
    ...
    # framework rewrite and not yet reintroduced. Skip until they are, like the
    # pg_bin helper.
    ...
    with node.connect() as c:
        c.sql("BEGIN")
        c.prepare("stmt", "SELECT a FROM foo ORDER BY a OFFSET $1")
        c.exec_prepared("stmt", 4999)
        c.sql("SELECT 'named portal'")
        c.sql("END")
    assert re.search(
    ...
    offset = node.current_log_position()
    with node.connect() as c:
        c.prepare("p1", "SELECT a, a, a FROM foo ORDER BY a OFFSET $1")
        c.exec_prepared("p1", 4993)
    log = node.log_since(offset)
    assert re.search(r"LOG:\s+temporary file:", log), "temporary file removed"
    assert not re.search(r"STATEMENT:", log), "no statement logged"
