...
def node(create_pg_module):
    ...
    skip_unless_injection_points(n)
...
def test_reindex_upsert(node, perm, table, index, conflict):
    ...
    s1, s2, s3 = node.background(), node.background(), node.background()
    ...
    f3 = s3.asql(f"REINDEX INDEX CONCURRENTLY test.{index}")
...
def test_reindex_partitioned_cache_inval(node):
    ...
    s1, s2 = node.background(), node.background()

    attach(s1, ANCESTORS)
    attach(s2, SWAP)

    f2 = s2.asql("REINDEX INDEX CONCURRENTLY test.tbl_partition_pkey")
