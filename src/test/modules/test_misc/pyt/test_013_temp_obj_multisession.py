...
def test_temp_obj_multisession(create_pg):
    ...
    psql1.sql("CREATE FUNCTION pg_temp.foo_id(r foo) RETURNS int LANGUAGE SQL AS 'SELECT r.val'")
    node.sql(f"ALTER FUNCTION {schema}.foo_id({schema}.foo) SET search_path = pg_catalog")
    ...
    with pytest.raises(
        LibpqError, match=r"cannot drop table .*\.foo2 because other objects depend on it"
    ):
    ...
    psql2 = node.background()
    psql2.sql(f"BEGIN; LOCK TABLE {schema}.foo2 IN ACCESS SHARE MODE;")
