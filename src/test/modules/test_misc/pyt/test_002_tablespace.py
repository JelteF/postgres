...
def test_tablespace(create_pg):
    ...
    # In-place tablespaces need the SET to persist to the CREATE, so they must
    # share a connection; CREATE TABLESPACE can't run in a transaction block,
    # so the statements are issued separately rather than as one string.
    c = node.connect()
