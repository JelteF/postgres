...
def test_replslot_single_user(create_pg):
    ...
    def single(queries):
        ...
        postgres(
            ...
            "-D", datadir, "postgres",
            input=queries, encoding="utf-8",
        )

    single(f"SELECT pg_create_logical_replication_slot('{SLOT_LOGICAL}', 'test_decoding')")
    ...
    single(f"SELECT pg_replication_slot_advance('{SLOT_LOGICAL}', pg_current_wal_lsn())")
    single(f"SELECT pg_replication_slot_advance('{SLOT_PHYSICAL}', pg_current_wal_lsn())")
    single(f"SELECT pg_copy_logical_replication_slot('{SLOT_LOGICAL}', 'slot_log_copy')")
    single(f"SELECT pg_copy_physical_replication_slot('{SLOT_PHYSICAL}', 'slot_phy_copy')")
