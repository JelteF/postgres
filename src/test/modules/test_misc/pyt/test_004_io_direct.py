...
def test_io_direct(create_pg, tmp_path):
    ...
    with node.connect() as conn:
        conn.sql("create table t1 as select 1 as i from generate_series(1, 10000)")
        conn.sql("create table t2count (i int)")
    node.sql_batch(
        "begin",
    ...
    with node.connect() as conn:
        conn.sql("update t1 set i = i")
        assert conn.sql("select count(*) from t1") == 10000, "read back from shared"
        assert conn.sql("select * from t2count") == 10000, "read back from local"

    node.stop("immediate")
    node.start()
    assert (
        node.sql("select count(*) from t1") == 10000
