...
def test_catcache_inval(create_pg):
    ...
    session = node.background()
    waker = node.background()
    ...
    paused = session.asql("SELECT foofunc(1)")
    ...
    node.sql("CREATE FUNCTION foofunc() RETURNS integer AS $$ SELECT 123 $$ LANGUAGE SQL")
