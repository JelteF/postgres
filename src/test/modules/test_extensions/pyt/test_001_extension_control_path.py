...
def test_extension_control_path(create_pg):
    ...
    # The server has been restarted above; everything below runs on it with no
    # further bounce, so share one connection for the superuser queries. (The
    # non-superuser checks and the SET-scoped sql_batch keep their own
    # connections so that session state does not leak.)
    conn = node.connect()

    conn.sql("CREATE USER user01")

    assert conn.sql("SHOW extension_control_path") == f"$system{sep}{ext_dir}{sep}{ext_dir2}"

    conn.sql(f"CREATE EXTENSION {ext_name}")
    conn.sql(f"CREATE EXTENSION {ext_name2}")
    ...
    with pytest.raises(LibpqError, match='extension "invalid" is not available'):
        conn.sql("CREATE EXTENSION invalid")
