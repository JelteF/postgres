...
def test_commit_ts_restart(create_pg):
    node = create_pg("committs_restart")
    node.stop()
    ...
    with pytest.raises(LibpqError, match="cannot retrieve commit timestamp for transaction"):
        conn.sql("SELECT pg_xact_commit_timestamp('0')")

    # BootstrapTransactionId and FrozenTransactionId succeed but are null.
    assert conn.sql("SELECT pg_xact_commit_timestamp('1')") is None
    assert conn.sql("SELECT pg_xact_commit_timestamp('2')") is None
    # FirstNormalTransactionId occurred during initdb, before commit
    # timestamps were enabled, so it is null too.
    assert conn.sql("SELECT pg_xact_commit_timestamp('3')") is None

    conn.sql("CREATE TABLE committs_test(x integer, y timestamp with time zone)")

    conn.sql("BEGIN")
    conn.sql("INSERT INTO committs_test(x, y) VALUES (1, current_timestamp)")
    xid = conn.sql("SELECT pg_current_xact_id()::xid")
    conn.sql("COMMIT")
    ...
    node.append_conf("track_commit_timestamp = off")
    node.stop("fast")
    ...
    conn = node.connect()
    conn.sql("BEGIN")
    conn.sql("INSERT INTO committs_test(x, y) VALUES (2, current_timestamp)")
    xid_disabled = conn.sql("SELECT pg_current_xact_id()")
    conn.sql("COMMIT")
