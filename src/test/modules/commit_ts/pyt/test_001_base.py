...
def test_commit_ts_survives_recovery(create_pg):
    ...
    # Create a table, compare now() to the commit TS of its xmin. These run on
    # one server with no bounce in between, so share a single connection.
    with node.connect() as conn:
        conn.sql("create table t as select now from (select now(), pg_sleep(1)) f")
        assert conn.sql(
