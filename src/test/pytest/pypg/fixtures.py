...
# Stash key for tracking the log file info at the start of each test
_log_info_key = pytest.StashKey[tuple]()
...
def pg(request, pg_server_module, remaining_timeout):
    ...
    """
    logpath = pg_server_module.datadir / "postgresql.log"
    if logpath.exists():
        start_pos = logpath.stat().st_size
    else:
        start_pos = 0
    request.node.stash[_log_info_key] = (logpath, start_pos)
...
def pytest_runtest_makereport(item, call):
    ...
    if _log_info_key not in item.stash:
        return

    logpath, start_pos = item.stash.pop(_log_info_key)
    ...
    if log_content.strip():
        report.sections.append(("Postgres log", log_content))
