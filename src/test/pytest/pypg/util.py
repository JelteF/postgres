...
def wait_until(
    ...
    100ms: most polled conditions become true almost immediately, and a fixed
    100ms interval wastes on average half of it per wait, across hundreds of
    waits in a suite. Pass ``interval`` to poll at a fixed rate instead (e.g.
    when each attempt is itself expensive).
    """
    start = time.time()
    end = start + timeout
