...
class PostgresServer:
    ...
    def default_db(self) -> str:
        ...
        server to a client program run with those PG* variables, where
        silently substituting a different database would be a surprise.
        """
    ...
    def connstr(self, *, dbname: str | None = None, **opts: object) -> str:
        ...
        ``default_db``. Extra keyword options (e.g. ``application_name``) are
        appended. Used for ``primary_conninfo`` on standbys and by replication
        clients.
        """
    ...
    def _conn_opts(self, **opts: Any) -> dict[str, Any]:
        ...
        A ``dbname`` of None means "whichever database this server defaults
        to", so the helpers taking an optional dbname can pass theirs straight
        through instead of resolving the fallback themselves.
        """
        if opts.get("dbname") is None:
            opts["dbname"] = self._default_db
