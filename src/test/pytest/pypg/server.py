...
class PostgresServer:
    ...
    def __init__(
        ...
        *,
        ...
        # Database that connect() and everything built on it use when the
        # caller doesn't name one (see the default_db property).
    ...
    def default_db(self) -> str:
        """The database used by connect(), and so by sql(), sql_oneshot(),
        poll_query_until() and the rest, when the caller names none.

        Assigning to it closes the cached sql() connection, so the next query
        ...
        Use it when a test does most of its work in a database other than
        postgres, instead of repeating ``dbname=`` at every call. An explicit
        ``dbname=`` still wins, so a test can reach back to another database at
        any point.

        Deliberately *not* applied to connstr() or connection_env(): those
        describe the server to something else -- a standby's primary_conninfo, a
        client program's environment -- where silently substituting a different
        database would be a surprise, and for logical replication a wrong one.
        """
        return self._default_db

    @default_db.setter
    def default_db(self, dbname: str) -> None:
        # The cached connection is attached to the old database, so it has to go
        # for the change to mean anything for sql().
        self.close_default_conn()
        self._default_db = dbname
    ...
    def poll_query_until(
        ...
        *params: Any,
        ...
        with self.connect(dbname=dbname or self.default_db) as conn:
            for _ in wait_until(
    ...
    def pg_recvlogical_upto(
        ...
        *,
        ...
        args = [
            ...
            slot_name,
            "--dbname",
            self.connstr(dbname=dbname or self.default_db),
            "--endpos",
            endpos,
            "--file",
            "-",
    ...
    def repeat_query(
        ...
        conn = self.connect(dbname=dbname or self.default_db)
    ...
    def connect(self, **opts: Any) -> PGconn:
        ...
        """
        opts.setdefault("dbname", self._default_db)
