...
def test_int_param_is_typed(conn):
    """An integer parameter carries its type, so overloads resolve.

    Left untyped the server sees "unknown" and cannot choose between
    generate_series(int, int), (bigint, bigint), (numeric, numeric) and so on,
    which used to force callers to write $1::int in the query.
    """
    assert conn.sql("SELECT count(*) FROM generate_series($1, $2) i", 1, 5) == 5


def test_int_param_reaches_an_int_argument(conn):
    """int4 is declared where the value fits, so a plain integer argument
    matches without an assignment cast."""
    assert conn.sql("SELECT div($1, $2)", 7, 2) == 3


def test_big_int_param_is_bigint(conn):
    """Values too large for int4 are declared bigint instead."""
    big = 2**40
    assert conn.sql("SELECT $1", big) == big


def test_huge_int_param_is_numeric(conn):
    """Python ints are unbounded, so past int8 the parameter becomes numeric."""
    huge = 2**64
    assert conn.sql("SELECT $1", huge) == huge


def test_bool_param_is_typed(conn):
    assert conn.sql("SELECT $1", True) is True
    assert conn.sql("SELECT $1", False) is False


def test_float_param_is_typed(conn):
    assert conn.sql("SELECT $1 * 2", 1.5) == 3.0


def test_string_param_is_left_for_the_server_to_resolve(conn):
    """Strings stay untyped on purpose, so context decides what they are.

    Here the same Python str is resolved as a regclass and as a pg_lsn without
    the test naming either type.
    """
    conn.sql("CREATE TEMPORARY TABLE param_target(a int)")

    assert conn.sql("SELECT pg_relation_filepath($1) IS NOT NULL", "param_target")
    assert conn.sql("SELECT pg_wal_lsn_diff($1, '0/0') > 0", "0/1")


def test_none_param_is_null(conn):
    """None becomes a SQL NULL.

    Like a string it is sent untyped, so it needs a context that determines the
    type -- coalesce's other argument here. A bare ``SELECT $1 IS NULL`` fails
    with "could not determine data type of parameter $1", which is the server
    being unable to type the query rather than anything wrong with the value.
    """
    assert conn.sql("SELECT coalesce($1, 'fallback')", None) == "fallback"
    assert conn.sql("SELECT $1::int IS NULL", None) is True
