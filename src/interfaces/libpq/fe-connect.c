...
connectDBStart(PGconn *conn)
{
	 ...
	 *
	 * Cancel requests are special though, they should only try one host, which
	 * is determined in PQcancelConnectStart. So leave these settings alone for
	 * cancel requests.
	 */
	if (!conn->cancelRequest) {
		conn->whichhost = -1;
		...
		case CONNECTION_CHECK_STANDBY:
			{
				 ...
				 * clean connection closure. See this for details:
...
keep_going:						/* We will come back to here until there is
	...
	if (conn->try_next_host)
	{
		...
		/*
		 * Cancel requests never have more hosts to try. They should only try a
		 * single one.
		 */
		if (conn->cancelRequest) {
			goto error_return;
