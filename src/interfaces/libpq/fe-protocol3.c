...
pqGetNegotiateProtocolVersion3(PGconn *conn)
{
	...
	for (int i = 0; i < num; i++)
	{
		...
		for (const pg_protocol_parameter *param = KnownProtocolParameters; param->name; param++)
		{
			...
			{
				libpq_append_conn_error(conn, "received invalid protocol negotiation message: server reported an unsupported parameter without a _pq_. prefix (\"%s\")", conn->workBuffer.data);
				goto failure;
			}
