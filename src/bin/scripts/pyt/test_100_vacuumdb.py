...
def test_table_analyze_complete_column_list(pg):
    ...
    try:
        with pg.log_contains(
            ...
            vacuumdb(
                "--analyze", "--table", "vactable(a, b)", "postgres", server=pg
            )
    finally:
        pg.sql("DROP TABLE vactable")
...
def test_table_analyze_only_partial_column_list(pg):
    ...
    try:
        with pg.log_contains(r"statement: ANALYZE public.vactable\(b\);"):
            vacuumdb(
                "--analyze-only", "--table", "vactable(b)", "postgres", server=pg
            )
    finally:
        pg.sql("DROP TABLE vactable")
...
def test_min_mxid_age_incorrect_value(pg):
    ...
    try:
        vacuumdb.check_all(
            "--table", "vactable", "--min-mxid-age", "0", "postgres",
            server=pg, exit_code=1,
        )
    finally:
        pg.sql("DROP TABLE vactable")
...
def test_min_xid_age_incorrect_value(pg):
    ...
    try:
        vacuumdb.check_all(
            "--table", "vactable", "--min-xid-age", "0", "postgres",
            server=pg, exit_code=1,
        )
    finally:
        pg.sql("DROP TABLE vactable")
...
def test_table_min_mxid_age(pg):
    ...
    try:
        with pg.log_contains(r"GREATEST.*relminmxid.*2147483000"):
            vacuumdb(
                "--table", "vactable", "--min-mxid-age", "2147483000", "postgres",
                server=pg,
            )
    finally:
        pg.sql("DROP TABLE vactable")
...
def test_missing_stats_only(pg):
    ...
    try:
        ...
        with pg.log_contains(r"statement: ANALYZE", times=0):
            vacuumdb(
                "--analyze-only", "--dry-run", "--missing-stats-only",
        ...
        with pg.log_contains(r"statement: ANALYZE"):
            vacuumdb(
                "--analyze-only", "--missing-stats-only",
                "-t", "regression_vacuumdb_test", "postgres",
                server=pg,
            )
        ...
        with pg.log_contains(r"statement: ANALYZE"):
            vacuumdb(
                "--analyze-in-stages", "--missing-stats-only",
                "-t", "regression_vacuumdb_test", "postgres",
                server=pg,
            )
        with pg.log_contains(r"statement: ANALYZE", times=0):
