...
def test_createdb_quote_handling(pg):
    ...
    createdb.check_all(
        "--lc-collate", "foo'; SELECT '1", "foobarq",
        exit_code=1,
        stdout=r"^$",
        stderr=(
...
def test_createdb_template_and_strategy(pg):
    ...
    with pg.log_contains(
        ...
        createdb(
            "--template", "foobar2", "--strategy", "wal_log", "foobar6", server=pg
        )
    with pg.log_contains(
