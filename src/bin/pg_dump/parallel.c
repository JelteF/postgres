 ...
 *
 * Returns false so that the Windows console handler allows default
 * processing (ExitProcess) to continue.  On Unix, called from
 * CancelBackendsAndExit which never returns.
 */
static bool
CancelBackends(void)
{
...
/*
 * Cancel callback for the cancel thread.  Cancels all backends and exits
 * using _exit() not exit(), because the latter will invoke atexit handlers
 * that can fail if we interrupted related code.
 */
pg_noreturn static bool
...
#ifdef WIN32
	setup_cancel_handler(NULL, CancelBackends);
#else
