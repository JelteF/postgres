...
def test_promote_nonexistent_directory(tmp_path):
    pg_ctl.check_all(
        "--pgdata", tmp_path / "nonexistent", "promote",
        exit_code=1, stderr=r"directory .* does not exist",
    )
...
def test_promote_of_non_standby_fails(create_pg):
    ...
    primary.stop()
    pg_ctl.check_all(
        "--pgdata", primary.datadir, "promote",
        exit_code=1, stderr=r"PID file .* does not exist",
    )
    ...
    pg_ctl.check_all(
        "--pgdata", primary.datadir, "promote",
        exit_code=1, stderr=r"not in standby mode",
    )
